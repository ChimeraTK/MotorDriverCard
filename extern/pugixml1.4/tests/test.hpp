...
bool test_node(
    const pugi::xml_node& node, const pugi::char_t* contents, const pugi::char_t* indent, unsigned int flags);
bool test_double_nan(double value);

#ifndef PUGIXML_NO_XPATH
bool test_xpath_string(const pugi::xpath_node& node, const pugi::char_t* query, pugi::xpath_variable_set* variables,
    const pugi::char_t* expected);
bool test_xpath_boolean(
    const pugi::xpath_node& node, const pugi::char_t* query, pugi::xpath_variable_set* variables, bool expected);
bool test_xpath_number(
    const pugi::xpath_node& node, const pugi::char_t* query, pugi::xpath_variable_set* variables, double expected);
...
#define TEST_XML_FLAGS(name, xml, flags)                                                                               \
  struct test_fixture_##name {                                                                                         \
    pugi::xml_document doc;                                                                                            \
                                                                                                                       \
    test_fixture_##name() { CHECK(doc.load(PUGIXML_TEXT(xml), flags)); }                                               \
                                                                                                                       \
   private:                                                                                                            \
...
#  define STRINGIZE(value)                                                                                             \
    "??" // MSVC 6.0 and CodeWarrior have troubles stringizing stuff with strings
         // w/escaping inside
#else
...
#define CHECK_STRING(value, expected)                                                                                  \
  CHECK_TEXT(test_string_equal(value, expected), STRINGIZE(value) " is not equal to " STRINGIZE(expected))
...
#define CHECK_NODE_EX(node, expected, indent, flags)                                                                   \
  CHECK_TEXT(test_node(node, expected, indent, flags), STRINGIZE(node) " contents does not match " STRINGIZE(expected))
...
#  define CHECK_XPATH_NUMBER_NAN_VAR(node, query, variables)                                                           \
    CHECK_TEXT(test_xpath_number_nan(node, query, variables),                                                          \
        STRINGIZE(query) " does not evaluate to NaN in context " STRINGIZE(node))
#  define CHECK_XPATH_NODESET_VAR(node, query, variables)                                                              \
    xpath_node_set_tester(pugi::xpath_query(query, variables).evaluate_node_set(node),                                 \
        CHECK_JOIN2(STRINGIZE(query) " does not evaluate to expected set in "                                          \
                                     "context " STRINGIZE(node),                                                       \
            __FILE__, __LINE__))
#  define CHECK_XPATH_FAIL_VAR(query, variables)                                                                       \
    CHECK_TEXT(test_xpath_fail_compile(query, variables), STRINGIZE(query) " should not compile")
...
#if(defined(_MSC_VER) && _MSC_VER == 1200) || (defined(__INTEL_COMPILER) && __INTEL_COMPILER == 800) ||                \
