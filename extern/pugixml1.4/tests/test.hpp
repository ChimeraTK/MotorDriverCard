...
inline bool test_node_name_value(const Node &node, const pugi::char_t *name,
                                 const pugi::char_t *value) {
  return test_string_equal(node.name(), name) &&
         test_string_equal(node.value(), value);
}

bool test_node(const pugi::xml_node &node, const pugi::char_t *contents,
               const pugi::char_t *indent, unsigned int flags);
...
bool test_xpath_boolean(const pugi::xpath_node &node, const pugi::char_t *query,
                        pugi::xpath_variable_set *variables, bool expected);
bool test_xpath_number(const pugi::xpath_node &node, const pugi::char_t *query,
                       pugi::xpath_variable_set *variables, double expected);
...
#define CHECK_NAME_VALUE(node, name, value)                                    \
  CHECK_TEXT(test_node_name_value(node, name, value),                          \
             STRINGIZE(node) " name/value do not match " STRINGIZE(            \
                 name) " and " STRINGIZE(value))
...
#define CHECK_XPATH_STRING_VAR(node, query, variables, expected)               \
  CHECK_TEXT(test_xpath_string(node, query, variables, expected),              \
             STRINGIZE(query) " does not evaluate to " STRINGIZE(              \
                 expected) " in context " STRINGIZE(node))
...
#define CHECK_XPATH_NUMBER_NAN_VAR(node, query, variables)                     \
  CHECK_TEXT(                                                                  \
      ...
      STRINGIZE(query) " does not evaluate to NaN in context " STRINGIZE(      \
          node))
