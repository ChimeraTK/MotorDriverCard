...
std::basic_string<wchar_t> xml_writer_string::as_wide() const {
  ...
  return std::basic_string<wchar_t>(
      static_cast<const wchar_t*>(static_cast<const void*>(contents.data())), contents.size() / sizeof(wchar_t));
}
...
std::basic_string<pugi::char_t> xml_writer_string::as_string() const {
  ...
  // round-trip pointer through void* to avoid pointer alignment warnings;
  // contents data should be heap allocated => safe to cast
  return std::basic_string<pugi::char_t>(static_cast<const pugi::char_t*>(static_cast<const void*>(contents.data())),
...
bool test_save_narrow(const pugi::xml_document& doc, unsigned int flags, pugi::xml_encoding encoding,
...
bool test_write_narrow(
    pugi::xml_node node, unsigned int flags, pugi::xml_encoding encoding, const char* expected, size_t length) {
