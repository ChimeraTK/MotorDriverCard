...
// information). I intentionally decided to directly write the numbers in the
// code and not define constants for each start and stop bit. These constants
// whould only tempt users to use them in their code and start bit-fiddeling
// themselves. In addition, having a header with so many constants would not
...
namespace mtca4u {
   ...
   *  from TMC429InputWord::getDATA() (the data payload sub word as described in
   * the data sheet).
   */
