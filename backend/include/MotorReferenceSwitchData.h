...
namespace mtca4u{
   ...
   *
   *  In case a switch is disabled the motor will not stop at the corresponding mechanical
   *  position and the switch is always reported to be inactive.
   *
   * If the switch is enabled the switch is inactive if the end positon is not reached and becomes
   * active if the switch position is reached. In case of an "active open" switch 
   *
   * @attention This class is only a container and does not check for data consistency. It is 
   * possible to set switches active, even though the corresponding  enabled bits are off.
   *
   */
  class MotorReferenceSwitchData : public MultiVariableWord{
  public:
    /// The default constructor initialises everyting to 0 (all switches inactive and disabled);
