...
public:
  ...
  virtual int
  getActualVelocity() = 0; ///< Get the actual velocity in steps per FIXME
  virtual unsigned int
  getActualAcceleration() = 0; ///< Get the actual acceleration in steps per
                               ///< square FIXME
  virtual unsigned int
  getMicroStepCount() = 0; ///< Get the micro step value (which units? what does
                           ///< it mean?)

  virtual DriverStatusData
  getStatus() = 0; ///< Get the status of the motor driver
  virtual unsigned int
  getDecoderReadoutMode() = 0; ///< Get the decoder readout mode
  virtual unsigned int
  getDecoderPosition() = 0; ///< Get the decoder position (which units?)
  ...
  setEnabled(bool enable =
                 true) = 0; ///< @deprecated Use setMotorCurrentEnabled instead.
  ...
  virtual void
  setPositiveReferenceSwitchCalibration(int calibratedPosition) = 0;
  virtual int getPositiveReferenceSwitchCalibration() = 0;
  virtual void
  setNegativeReferenceSwitchCalibration(int calibratedPosition) = 0;
  virtual int getNegativeReferenceSwitchCalibration() = 0;

  virtual bool
  isEnabled() = 0; ///< @deprecated Use isMotorCurrentEnabled instead.
  ...
  virtual MotorReferenceSwitchData
  getReferenceSwitchData() = 0; ///< Get information about both reference
                                ///< switches of this Motor
  ...
  MC_DECLARE_SIGNED_SET_GET_VALUE(TargetPosition,
                                  steps); ///< Get the target position in steps
  MC_DECLARE_SET_GET_VALUE(MinimumVelocity,
                           stepsPerFIXME); ///< Get minimum velocity in FIXME
  MC_DECLARE_SET_GET_VALUE(MaximumVelocity,
                           stepsPerFIXME); ///< Get maximum velocity in FIXME
  MC_DECLARE_SIGNED_SET_GET_VALUE(
      TargetVelocity, stepsPerFIXME); ///< Get target velocity in FIXME
  MC_DECLARE_SET_GET_VALUE(
      MaximumAcceleration,
      stepsPerSquareFIXME); ///< Get the maximim acceleration in FIXME
  MC_DECLARE_SET_GET_VALUE(PositionTolerance,
                           steps); ///< Get the position tolerance in steps
