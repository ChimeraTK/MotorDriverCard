...
public:
  ...
  void
  setStallGuardControlData(StallGuardControlData const &stallGuardControlData);
...
private:
  ...
  template <class T> T readTypedRegister();
  ...
  static double
  calculateConversionFactor(MotorControlerConfig const &motorControlerConfig);
  ...
  inline unsigned int
  readRegisterAccessor(ChimeraTK::ScalarRegisterAccessor<int32_t> &readValue);
};
