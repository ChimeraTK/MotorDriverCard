...
namespace mtca4u {
   ...
   * MotorDriverCardFactory. This ensures that only one instance per device is
   * created, which allows thread safe access to the motors.
   */
  class MotorDriverCardImpl : public MotorDriverCardExpert {
   public:
    ...
    void setCoverPositionAndLength(CoverPositionAndLength const& coverPositionAndLength);
    ...
    void setStepperMotorGlobalParameters(StepperMotorGlobalParameters const& stepperMotorGlobalParameters);
    StepperMotorGlobalParameters getStepperMotorGlobalParameters();

    void setInterfaceConfiguration(InterfaceConfiguration const& interfaceConfiguration);
    ...
    /// the programmer should not have to touch this.
    void setPositionCompareInterruptData(PositionCompareInterruptData const& positionCompareInterruptData);
    PositionCompareInterruptData getPositionCompareInterruptData();

    void powerDown();
    ReferenceSwitchData getReferenceSwitchData();

    /** Get the 8 bit status word which is always send a the most significant byte
     * of the TMC429 readback word.*/
   ...
   private:
    ...
    /// Checks the firmware version and throws an exception if it is not valid
    /// (too old or too new)
    void checkFirmwareVersion();

    /// The constructor requires a working version of MtcaDevice in addition
    /// to the configuration. These are provided by the factory. The constructor
    /// is private, so the class can only be generated by the factory.
    MotorDriverCardImpl(boost::shared_ptr<ChimeraTK::Device> const& device, std::string const& moduleName,
