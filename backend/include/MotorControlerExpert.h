...
public:
  ...
  getStallGuardValue() = 0; ///< Get the stall guard value  (which units? what
                            ///< does it mean? Expert?)
  virtual unsigned int
  getCoolStepValue() = 0; ///< Get the CoolStepValue (in units?, what does it
                          ///< mean? Expert?)

  MCX_DECLARE_SET_GET_TYPED_REGISTER(AccelerationThresholdData,
                                     accelerationThresholdData);
  MCX_DECLARE_SET_GET_TYPED_REGISTER(ProportionalityFactorData,
                                     proportionalityFactorData);
  MCX_DECLARE_SET_GET_TYPED_REGISTER(ReferenceConfigAndRampModeData,
                                     referenceConfigAndRampModeData);
  MCX_DECLARE_SET_GET_TYPED_REGISTER(InterruptData, interruptData);
  MCX_DECLARE_SET_GET_TYPED_REGISTER(DividersAndMicroStepResolutionData,
                                     dividersAndMicroStepResolutionData);

  virtual void
  setDriverControlData(DriverControlData const &driverControlData) = 0;
  virtual void
  setChopperControlData(ChopperControlData const &chopperControlData) = 0;
  virtual void
  setCoolStepControlData(CoolStepControlData const &coolStepControlData) = 0;
  virtual void setStallGuardControlData(
      StallGuardControlData const &stallGuardControlData) = 0;
  virtual void
  setDriverConfigData(DriverConfigData const &driverConfigData) = 0;
