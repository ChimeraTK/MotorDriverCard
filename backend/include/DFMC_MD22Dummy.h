 ...
 * \li All registers have full 32 bit data when written directly. Most registers
 * are not write protected FIXME. \li The following registers are automatically
 * updated to the TMC429 controler SPI values after an SPI write. They are write
 * protected: <ul> <li> ACTUAL_POSITION </li> <li> ACTUAL_VELOCITY </li> <li>
 * ACTUAL_ACCELERATION </li> <li> MICRO_STEP_COUNT </li>
 *  </ul>
 *  The updated values are truncated to 24 bit. The 24 bit are not truncated
 * according to the bit patterns of the TMC429 chip. \li Test words with content
 * calculated from register index for SPI registers. \li Initialisation to start
 * values of TMC429 \li An spi address space of 0x40 words is defined, which can
 * be writen via the SPI_CTRL register. \li A read request to the SPI_CTRL
 * register updates the value in the SPI_CTRL_B register
 */
...
public:
   ...
   *
   */
  DFMC_MD22Dummy(std::string const &mapFileName,
                 std::string const &tmc429ControllerModuleName);
  void open();

  /** Writes the test pattern to all registers.
   *  controlerSpiAddress*controlerSpiAddress+13 for the controler SPI address
   * space. 3*address*address+17 for the PCIe addresses. spiAddress*spiAddress +
   * 7 + motorID for the drivers SPI addres spaces (motor 0 and 1)
   *
   *  Exception: The registers from the standard register set are set to their
   * no1rmal values.
   */
  ...
  unsigned int readDriverSpiRegister(unsigned int motorID,
                                     unsigned int driverSpiAddress);

  /** For testing all driver registers can be set to test values (also those
   * initialised by the config)
   */
  void setDriverSpiRegistersForTesting();

  /** The dummy does not react on the SPI write registers if this flag is set
   * true. The limitToNTimeouts allows to return to normal operation
   * (causesSpiTimeouts = false) when the specified number if timeouts has been
   * caused. If the limitToNTimeouts is left at 0 there will be an unlimited
   * number of timeouts.
   */
  void causeSpiTimeouts(bool causeTimeouts, unsigned int limitToNTimeouts = 0);

  /** The dummy always reports an SPI error in the sync registers if this flag
   * is set true.
   */
  void causeSpiErrors(bool causeErrors);

  /** Overwrite the firmware version for testing. To restore the dummy default
   * use resetFirmwareVersion().
   */
  ...
  /** Set the simulated SPI delay for the TMC429 controller chip in
   * microseconds.
   */
   ...
   */
  uint32_t readTMC260Register(uint32_t motorID,
                              TMC260Register configuredRegister);

  static boost::shared_ptr<ChimeraTK::DeviceBackend>
  createInstance(std::string host, std::string interface,
...
private:
  ...
  void writeContentToControlerSpiRegister(unsigned int content,
                                          unsigned int controlerSpiAddress);
  void
  writeControlerSpiContentToReadbackRegister(unsigned int controlerSpiAddress);
  ...
  /** In the DFMC_MD22 some registers of the controler SPI address space have a
   * copy in the PCIe address space, which is updated by a loop when the FPGA is
   * idle. The dummy simulates this behaviour.
   */
  ...
  /** Set the first word of an address range and make it constant. Intended only
   * for single word address ranges. In larger address ranges only the first
   * word is set and made const.
   */
  ...
  void driverSpiActions(unsigned int ID, unsigned int spiAddress,
  ...
  unsigned int _nSpiTimeoutsLeft; // when this counter reaches 0 it sets
                                  // _causeSpiTimeouts back to false
  ...
  bool checkStatusOfRWBit(const TMC429InputWord &inputSPMessage,
                          uint32_t bitStatus);
  TMC429OutputWord frameTMC429SPIResponse(TMC429InputWord &inputSpi);

  // If the timeout counter is not 0 it is decremented. Then it goes from 1 to 0
  // the _causeSpiTimeouts flag is set to false. It returns the
  // _causeSpiTimeouts flag from before the reset, so the answer is true if the
  // counter was 1, even tough the flag has been cleared.
...
public:
  DFMC_MD22DummyRegisterer() {
    ...
    ChimeraTK::BackendFactory::getInstance().registerBackendType(
        "dfmc_md22dummy", "", &DFMC_MD22Dummy::createInstance,
