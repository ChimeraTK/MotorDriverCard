...
#define CARD_CONFIG_ADD_REGISTER(VALUE)                                        \
  cardConfigFiller.addParameter(#VALUE, motorDriverCardConfig.VALUE,           \
                                defaultCardConfig.VALUE)

#define CARD_CONFIG_ADD_PARAMETER(VALUE)                                       \
  cardConfigFiller.addParameter(#VALUE, motorDriverCardConfig.VALUE,           \
                                defaultCardConfig.VALUE, "Parameter")

#define CONTROLER_CONFIG_ADD_REGISTER(VALUE)                                   \
  controlerConfigFiller.addParameter(#VALUE, motorControlerConfig.VALUE,       \
...
MotorDriverCardConfig MotorDriverCardConfigXML::read(std::string fileName) {
  ...
  setValueIfFound("coverPositionAndLength", cardConfig.coverPositionAndLength,
                  cardConfigXML);
  setValueIfFound("datagramHighWord", cardConfig.datagramHighWord,
                  cardConfigXML);
  setValueIfFound("datagramLowWord", cardConfig.datagramLowWord, cardConfigXML);
  setValueIfFound("interfaceConfiguration", cardConfig.interfaceConfiguration,
                  cardConfigXML);
  setValueIfFound("positionCompareInterruptData",
                  cardConfig.positionCompareInterruptData, cardConfigXML);
  setValueIfFound("positionCompareWord", cardConfig.positionCompareWord,
                  cardConfigXML);
  setValueIfFound("stepperMotorGlobalParameters",
                  cardConfig.stepperMotorGlobalParameters, cardConfigXML);
  setValueIfFound("controlerSpiWaitingTime", cardConfig.controlerSpiWaitingTime,
                  cardConfigXML, "Parameter");

  for (size_t i = 0; i < cardConfig.motorControlerConfigurations.size(); ++i) {
    std::stringstream motorIDAsString;
    motorIDAsString << i;
    pugi::xml_node controlerConfigXML = cardConfigXML.find_child_by_attribute(
        "MotorControlerConfig", "motorID", motorIDAsString.str().c_str());
    if (controlerConfigXML) {
      cardConfig.motorControlerConfigurations[i] =
          parseControlerConfig(controlerConfigXML);
    }
  }
...
void MotorDriverCardConfigXML::setValueIfFound(std::string const &parameterName,
  ...
  pugi::xml_node parameterNode = parentNode.find_child_by_attribute(
      tagName.c_str(), "name", parameterName.c_str());
  parameterContent = parameterNode.attribute("value").as_uint(
      parameterContent /* = default vale*/);
}
...
void MotorDriverCardConfigXML::setValueIfFound(std::string const &parameterName,
  ...
  pugi::xml_node parameterNode = parentNode.find_child_by_attribute(
      tagName.c_str(), "name", parameterName.c_str());
  parameterContent = parameterNode.attribute("value").as_int(
      parameterContent /* = default vale*/);
}
...
void MotorDriverCardConfigXML::setValueIfFound(
    std::string const &registerName, TMC429InputWord &inputWord,
    pugi::xml_node const &parentNode) {
  pugi::xml_node registerNode = parentNode.find_child_by_attribute(
      "Register", "name", registerName.c_str());
  unsigned int readValue = registerNode.attribute("value").as_int(
      inputWord.getDATA() /* = default vale*/);
  inputWord.setDATA(readValue);
}

void MotorDriverCardConfigXML::setValueIfFound(
    std::string const &registerName, TMC260Word &inputWord,
    pugi::xml_node const &parentNode) {
  pugi::xml_node registerNode = parentNode.find_child_by_attribute(
      "Register", "name", registerName.c_str());
  unsigned int readValue = registerNode.attribute("value").as_int(
      inputWord.getPayloadData() /* = default vale*/);
  inputWord.setPayloadData(readValue);
}

MotorControlerConfig MotorDriverCardConfigXML::parseControlerConfig(
    pugi::xml_node const &controlerConfigXML) {
  MotorControlerConfig controlerConfig;

  setValueIfFound("accelerationThresholdData",
                  controlerConfig.accelerationThresholdData,
                  controlerConfigXML);
  // setValueIfFound("actualPosition", controlerConfig.actualPosition,
  // controlerConfigXML);
  setValueIfFound("chopperControlData", controlerConfig.chopperControlData,
                  controlerConfigXML);
  setValueIfFound("coolStepControlData", controlerConfig.coolStepControlData,
                  controlerConfigXML);
  setValueIfFound("decoderReadoutMode", controlerConfig.decoderReadoutMode,
                  controlerConfigXML);
  setValueIfFound("dividersAndMicroStepResolutionData",
                  controlerConfig.dividersAndMicroStepResolutionData,
                  controlerConfigXML);
  setValueIfFound("driverConfigData", controlerConfig.driverConfigData,
                  controlerConfigXML);
  setValueIfFound("driverControlData", controlerConfig.driverControlData,
                  controlerConfigXML);
  setValueIfFound("enabled", controlerConfig.enabled, controlerConfigXML);
  setValueIfFound("interruptData", controlerConfig.interruptData,
                  controlerConfigXML);
  setValueIfFound("maximumAcceleration", controlerConfig.maximumAcceleration,
                  controlerConfigXML);
  setValueIfFound("maximumVelocity", controlerConfig.maximumVelocity,
                  controlerConfigXML);
  setValueIfFound("microStepCount", controlerConfig.microStepCount,
                  controlerConfigXML);
  setValueIfFound("minimumVelocity", controlerConfig.minimumVelocity,
                  controlerConfigXML);
  setValueIfFound("positionTolerance", controlerConfig.positionTolerance,
                  controlerConfigXML);
  setValueIfFound("proportionalityFactorData",
                  controlerConfig.proportionalityFactorData,
                  controlerConfigXML);
  setValueIfFound("referenceConfigAndRampModeData",
                  controlerConfig.referenceConfigAndRampModeData,
                  controlerConfigXML);
  setValueIfFound("stallGuardControlData",
                  controlerConfig.stallGuardControlData, controlerConfigXML);
  setValueIfFound("targetPosition", controlerConfig.targetPosition,
                  controlerConfigXML);
  setValueIfFound("targetVelocity", controlerConfig.targetVelocity,
                  controlerConfigXML);
  setValueIfFound("driverSpiWaitingTime", controlerConfig.driverSpiWaitingTime,
                  controlerConfigXML, "Parameter");
...
void MotorDriverCardConfigXML::write(
    std::string fileName, MotorDriverCardConfig const &motorDriverCardConfig) {
  write(fileName, motorDriverCardConfig, false);
}

void MotorDriverCardConfigXML::writeSparse(
    std::string fileName, MotorDriverCardConfig const &motorDriverCardConfig) {
...
void MotorDriverCardConfigXML::write(
  ...
  for (unsigned int i = 0;
       i < motorDriverCardConfig.motorControlerConfigurations.size(); ++i) {
    MotorControlerConfig defaultControlerConfig;
    MotorControlerConfig motorControlerConfig =
        motorDriverCardConfig.motorControlerConfigurations[i];
    ...
    pugi::xml_node controlerConfigXML =
        cardConfigXML.append_child("MotorControlerConfig");
...
void MotorDriverCardConfigXML::checkForOldInvalidTagName(
    std::string const &parameterName, pugi::xml_node const &parentNode,
    std::string const &tagName) {
  pugi::xml_node parameterNode = parentNode.find_child_by_attribute(
      tagName.c_str(), "name", parameterName.c_str());
  if (parameterNode) {
...
MotorDriverCardConfigXML::NodeFiller::NodeFiller(pugi::xml_node &node,
                                                 bool sparse)
    : _node(node), _writeAlways(!sparse) {}

void MotorDriverCardConfigXML::NodeFiller::addParameter(
    std::string const &parameterName, unsigned int value,
    unsigned int defaultValue, std::string const &tagName) {
...
void MotorDriverCardConfigXML::NodeFiller::addParameter(
    std::string const &parameterName, int value, int defaultValue,
    std::string const &tagName) {
  if (_writeAlways || (value != defaultValue)) {
    pugi::xml_node parameterNode = _node.append_child(tagName.c_str());
    parameterNode.append_attribute("name") = parameterName.c_str();
...
void MotorDriverCardConfigXML::NodeFiller::addParameter(
    std::string const &registerName, TMC429InputWord const &value,
    TMC429InputWord const &defaultValue) {
  addParameter(registerName, value.getDATA(), defaultValue.getDATA());
}

void MotorDriverCardConfigXML::NodeFiller::addParameter(
    std::string const &registerName, TMC260Word const &value,
    TMC260Word const &defaultValue) {
  addParameter(registerName, value.getPayloadData(),
               defaultValue.getPayloadData());
}

void MotorDriverCardConfigXML::NodeFiller::addParameter(
    std::string const &parameterName, bool value, bool defaultValue,
    std::string const &tagName) {
  if (_writeAlways || (value != defaultValue)) {
    pugi::xml_node parameterNode = _node.append_child(tagName.c_str());
    parameterNode.append_attribute("name") = parameterName.c_str();
...
std::string
MotorDriverCardConfigXML::NodeFiller::toHexString(unsigned int value) {
  std::stringstream s;
