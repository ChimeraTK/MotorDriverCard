...
#define DEFINE_ADDRESS_RANGE(NAME, ADDRESS_STRING, MOD_NAME)                   \
  _registerMapping->getRegisterInfo(ADDRESS_STRING, registerInformation,       \
                                    MOD_NAME);                                 \
  AddressRange NAME(registerInformation.bar, registerInformation.address,      \
...
DFMC_MD22Dummy::DFMC_MD22Dummy(std::string const &mapFileName,
                               std::string const &tmc429ControllerModuleName)
    : DummyBackend(mapFileName),
      _controlerSpiAddressSpace(tmc429::SIZE_OF_SPI_ADDRESS_SPACE, 0),
      _controlerSpiWriteAddress(0), _controlerSpiBar(0),
      _controlerSpiReadbackAddress(0), _controlerSpiSyncAddress(0),
      _powerIsUp(true), _driverSPIs(0), _causeSpiTimeouts(false),
      _nSpiTimeoutsLeft(0), _causeSpiErrors(false),
...
DFMC_MD22Dummy::DriverSPI::DriverSPI()
    : addressSpace(0), bar(0), pcieWriteAddress(0), pcieSyncAddress(0) {}
...
void DFMC_MD22Dummy::open() {
  ...
  DEFINE_ADDRESS_RANGE(controlerSpiWriteAddressRange,
                       CONTROLER_SPI_WRITE_ADDRESS_STRING, _moduleName);
  _controlerSpiWriteAddress = registerInformation.address;
  _controlerSpiBar = registerInformation.bar;

  _registerMapping->getRegisterInfo(CONTROLER_SPI_READBACK_ADDRESS_STRING,
                                    registerInformation, _moduleName);
  if (_controlerSpiBar != registerInformation.bar) {
    throw MotorDriverException(
        "SPI write and readback address must be in the same bar",
        MotorDriverException::SPI_ERROR);
  }
  _controlerSpiReadbackAddress = registerInformation.address;
  setReadOnly(registerInformation.bar, registerInformation.address,
              registerInformation.nElements);

  _registerMapping->getRegisterInfo(CONTROLER_SPI_SYNC_ADDRESS_STRING,
                                    registerInformation, _moduleName);
  if (_controlerSpiBar != registerInformation.bar) {
    throw MotorDriverException(
        "SPI write and sync address must be in the same bar",
        MotorDriverException::SPI_ERROR);
  }
  _controlerSpiSyncAddress = registerInformation.address;

  setWriteCallbackFunction(
      controlerSpiWriteAddressRange,
      boost::bind(&DFMC_MD22Dummy::handleControlerSpiWrite, this));
  ...
  for (unsigned int id = 0; id < N_MOTORS_MAX; ++id) {
    ...
    DEFINE_ADDRESS_RANGE(
        actualAccelerationAddressRange,
    ...
    _driverSPIs[id].bar =
        static_cast<unsigned int>(driverSpiWriteAddressRange.bar);
...
void DFMC_MD22Dummy::resetFirmwareVersion() {
  setConstantPCIeRegister(PROJECT_VERSION_ADDRESS_STRING,
                          MINIMAL_FIRMWARE_VERSION | 0xFFFF);
}

void DFMC_MD22Dummy::setConstantPCIeRegister(std::string registerName,
                                             int32_t content) {
  ChimeraTK::RegisterInfoMap::RegisterInfo
      registerInformation; // variable neede in the DEFINE_ADDRESS_RANGE macro
...
void DFMC_MD22Dummy::setDriverSpiRegistersForTesting(unsigned int motorID) {
  for (unsigned int address = 0;
       address < _driverSPIs[motorID].addressSpace.size(); ++address) {
    _driverSPIs[motorID].addressSpace[address] =
        tmc260::testWordFromSpiAddress(address, motorID);
  }
}

void DFMC_MD22Dummy::setControlerSpiRegistersForOperation() {
  for_each(_controlerSpiAddressSpace.begin(), _controlerSpiAddressSpace.end(),
           boost::lambda::_1 = 0);

  // Information from the data sheet:
  size_t controlerSpiAddress =
      spiAddressFromSmdaIdxJdx(SMDA_COMMON, JDX_CHIP_VERSION);
  _controlerSpiAddressSpace[controlerSpiAddress] = CONTROLER_CHIP_VERSION;

  // the CLK2_DIV subword is 15, all other bits stay 0
  controlerSpiAddress = spiAddressFromSmdaIdxJdx(
      SMDA_COMMON, JDX_STEPPER_MOTOR_GLOBAL_PARAMETERS);
  StepperMotorGlobalParameters globalParameters;
  globalParameters.setClk2_div(15);
  _controlerSpiAddressSpace.at(controlerSpiAddress) =
      globalParameters.getDATA();
...
void DFMC_MD22Dummy::handleControlerSpiWrite() {
  ...
  if (inputWord.getRW() == RW_READ) {
    ...
    boost::this_thread::sleep(
        boost::posix_time::microseconds(2 * _microsecondsControllerSpiDelay));
  } else {
    writeContentToControlerSpiRegister(inputWord.getDATA(),
                                       controlerSpiAddress);
    // it takes some time to perform this in firmware. Implement a short delay,
    // which proabaly will cause rescheduling.
    boost::this_thread::sleep(
        boost::posix_time::microseconds(_microsecondsControllerSpiDelay));
  }
...
void DFMC_MD22Dummy::handleDriverSpiWrite(unsigned int ID) {
  ...
  boost::this_thread::sleep(
      boost::posix_time::microseconds(_microsecondsDriverSpiDelay));

  _driverSPIs[ID].addressSpace.at(spiAddress) =
      writtenSpiWord & payloadDataMask;
...
uint32_t DFMC_MD22Dummy::readTMC260Register(uint32_t motorID,
                                            TMC260Register configuredRegister) {
  auto registerAddress = static_cast<unsigned int>(configuredRegister);
  auto registerDataPayload =
      _driverSPIs[motorID].addressSpace.at(registerAddress);
...
TMC429OutputWord
DFMC_MD22Dummy::frameTMC429SPIResponse(TMC429InputWord &inputSpi) {
  return TMC429OutputWord(_controlerSpiAddressSpace.at(inputSpi.getADDRESS()));
}

void DFMC_MD22Dummy::driverSpiActions(unsigned int ID, unsigned int spiAddress,
...
void DFMC_MD22Dummy::writeContentToControlerSpiRegister(
    unsigned int content, unsigned int controlerSpiAddress) {
  if (_powerIsUp) {
    _controlerSpiAddressSpace.at(controlerSpiAddress) = content;
  }
}

void DFMC_MD22Dummy::writeControlerSpiContentToReadbackRegister(
...
void DFMC_MD22Dummy::triggerActionsOnControlerSpiWrite(
    TMC429InputWord const &inputWord) {
...
void DFMC_MD22Dummy::powerUp() {
  unsigned int powerDownControlerSpiAddress =
      spiAddressFromSmdaIdxJdx(SMDA_COMMON, JDX_POWER_DOWN);
...
void DFMC_MD22Dummy::synchroniseFpgaWithControlerSpiRegisters() {
  for (unsigned int i = 0; i < N_MOTORS_MAX; ++i) {
    writeControlerSpiRegisterToFpga(i, IDX_ACTUAL_POSITION,
                                    ACTUAL_POSITION_SUFFIX);
    writeControlerSpiRegisterToFpga(i, IDX_ACTUAL_VELOCITY,
                                    ACTUAL_VELOCITY_SUFFIX);
    writeControlerSpiRegisterToFpga(i, IDX_ACTUAL_ACCELERATION,
                                    ACTUAL_ACCELETATION_SUFFIX);
    // writeAccelerationThresholdToFpgs( i );
    writeControlerSpiRegisterToFpga(i, IDX_MICRO_STEP_COUNT,
                                    MICRO_STEP_COUNT_SUFFIX);
  }
}

void DFMC_MD22Dummy::writeControlerSpiRegisterToFpga(unsigned int ID,
                                                     unsigned int IDX,
                                                     std::string suffix) {
  ...
  _registerMapping->getRegisterInfo(registerName, registerInformation,
                                    _moduleName);
  writeRegisterWithoutCallback(registerInformation.bar,
                               registerInformation.address,
                               _controlerSpiAddressSpace[controlerSpiAddress]);
}

unsigned int
DFMC_MD22Dummy::readDriverSpiRegister(unsigned int motorID,
...
boost::shared_ptr<ChimeraTK::DeviceBackend>
DFMC_MD22Dummy::createInstance(std::string /*host*/, std::string instance,
  ...
  return (returnInstance<DFMC_MD22Dummy>(
      instance, convertPathRelativeToDmapToAbs(mapFile),
