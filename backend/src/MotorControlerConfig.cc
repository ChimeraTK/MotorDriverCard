...
MotorControlerConfig::MotorControlerConfig()
    : accelerationThresholdData(0), // actualPosition(0),
      ...
      dividersAndMicroStepResolutionData(
          DIVIDIERS_AND_MICRO_STEP_RESOLUTION_DEFAULT),
      driverConfigData(DRIVER_CONFIG_DEFAULT),
      driverControlData(DRIVER_CONTROL_DEFAULT), enabled(false),
      interruptData(0), maximumAcceleration(MAXIMUM_ACCELERATION_DEFAULT),
      maximumVelocity(MAXIMUM_VELOCITY_DEFAULT), microStepCount(0),
      minimumVelocity(MINIMUM_VELOCITY_DEFAULT), positionTolerance(0),
      ...
      stallGuardControlData(STALL_GUARD_CONTROL_DEFAULT), targetPosition(0),
      targetVelocity(0), driverSpiWaitingTime(DRIVER_SPI_WAITING_TIME_DEFAULT) {
}
...
bool MotorControlerConfig::operator==(MotorControlerConfig const &right) const {
  return ((accelerationThresholdData == right.accelerationThresholdData) &&
          ...
          (dividersAndMicroStepResolutionData ==
           right.dividersAndMicroStepResolutionData) &&
          ...
          (referenceConfigAndRampModeData ==
           right.referenceConfigAndRampModeData) &&
