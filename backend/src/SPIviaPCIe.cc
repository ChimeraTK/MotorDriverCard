...
SPIviaPCIe::SPIviaPCIe(boost::shared_ptr<ChimeraTK::Device> const &device,
    ...
    : _writeRegister(device->getScalarRegisterAccessor<int32_t>(
      ...
      _synchronisationRegister(device->getScalarRegisterAccessor<int32_t>(
          ...
          {ChimeraTK::AccessMode::raw})),
      _readbackRegister(device->getScalarRegisterAccessor<int32_t>(
          moduleName + "/" + readbackRegisterName, 0,
          {ChimeraTK::AccessMode::raw})),
      _spiWaitingTime(spiWaitingTime), _moduleName(moduleName),
      _writeRegisterName(writeRegisterName),
      _syncRegisterName(syncRegisterName), _spiMutex() {}
...
void SPIviaPCIe::write(int32_t spiCommand) {
  ...
  errorDetails << "PCIe register " << _moduleName << "." << _writeRegisterName
               << ", sync register " << _moduleName << "." << _syncRegisterName
               << "= 0x" << std::hex << _synchronisationRegister
               << ", spi command 0x" << std::hex << spiCommand << std::dec;
  switch (_synchronisationRegister) {
  case SPI_SYNC_OK:
    break;
  case SPI_SYNC_REQUESTED:
    throw MotorDriverException(std::string("Timeout writing via SPI, ") +
                                   errorDetails.str(),
                               MotorDriverException::SPI_TIMEOUT);
  case SPI_SYNC_ERROR:
  default:
    throw MotorDriverException(std::string("Error writing via SPI, ") +
                                   errorDetails.str(),
