...
namespace mtca4u {
MotorDriverCardImpl::MotorDriverCardImpl(
  ...
  _controlerSPI.reset(new TMC429SPI(
      device, moduleName, CONTROLER_SPI_WRITE_ADDRESS_STRING,
  ...
  setPositionCompareInterruptData(
      cardConfiguration.positionCompareInterruptData);
  setPositionCompareWord(cardConfiguration.positionCompareWord);
  setStepperMotorGlobalParameters(
      cardConfiguration.stepperMotorGlobalParameters);
  ...
  for (unsigned int i = 0; i < _motorControlers.size(); ++i) {
    _motorControlers[i].reset(new MotorControlerImpl(
        i, device, moduleName, _controlerSPI,
        cardConfiguration.motorControlerConfigurations[i]));
  }
}

boost::shared_ptr<MotorControler>
MotorDriverCardImpl::getMotorControler(unsigned int motorControlerID) {
  try {
...
CoverPositionAndLength MotorDriverCardImpl::getCoverPositionAndLength() {
  return CoverPositionAndLength(
      _controlerSPI->read(SMDA_COMMON, JDX_COVER_POSITION_AND_LENGTH)
          .getDATA());
}
...
StepperMotorGlobalParameters
MotorDriverCardImpl::getStepperMotorGlobalParameters() {
  return StepperMotorGlobalParameters(
      _controlerSPI->read(SMDA_COMMON, JDX_STEPPER_MOTOR_GLOBAL_PARAMETERS)
          .getDATA());
}
...
InterfaceConfiguration MotorDriverCardImpl::getInterfaceConfiguration() {
  return InterfaceConfiguration(
      _controlerSPI->read(SMDA_COMMON, JDX_INTERFACE_CONFIGURATION).getDATA());
}
...
MotorDriverCardImpl::getPositionCompareInterruptData() {
  return PositionCompareInterruptData(
      _controlerSPI->read(SMDA_COMMON, JDX_POSITION_COMPARE_INTERRUPT)
          .getDATA());
}
...
void MotorDriverCardImpl::checkFirmwareVersion() {
  ChimeraTK::ScalarRegisterAccessor<int32_t> firmwareVersion =
      _device->getScalarRegisterAccessor<int32_t>(
          _moduleName + "/" + PROJECT_VERSION_ADDRESS_STRING);
  firmwareVersion.read();

  // only allow firmware versions with the same major number
  uint32_t maxFirmwareVersion =
      (MINIMAL_FIRMWARE_VERSION & 0xFF000000) | 0x00FFFFFF;

  // check the version
  if ((static_cast<uint32_t>(firmwareVersion) <
       static_cast<uint32_t>(MINIMAL_FIRMWARE_VERSION)) ||
    ...
    errorMessage << "MotorDriverCardImpl detected wrong firmware version. "
                 ...
                 << ", minumum required version is 0x"
                 << MINIMAL_FIRMWARE_VERSION
                 << ", maximum allowed version is 0x" << maxFirmwareVersion
                 << std::dec << ".";
    std::cerr << errorMessage.str() << std::endl;
    throw MotorDriverException(errorMessage.str(),
                               MotorDriverException::WRONG_FIRMWARE_VERSION);
  }
}
