...
#define RAW_ACCESSOR_FROM_SUFFIX(MODULE_NAME, SUFFIX)                          \
  device->getScalarRegisterAccessor<int32_t>(                                  \
      MODULE_NAME + "/" + createMotorRegisterName(_id, SUFFIX), 0,             \
      {ChimeraTK::AccessMode::raw})

// Macros which cover more than one line are not good for the code coverage
// test, as only the macro call is checked. In this case it is probably ok
// because we gain a lot of code because we have to define many variables, AND
// there is only one line per function without branching, which is covered by
// the function coverage test (not the line coverage test). Otherwise we would
// need a complete test which checks the macro.
...
namespace mtca4u {
MotorControlerImpl::MotorControlerImpl(
    unsigned int ID, boost::shared_ptr<ChimeraTK::Device> const &device,
    std::string const &moduleName,
    boost::shared_ptr<TMC429SPI> const &controlerSPI,
    MotorControlerConfig const &motorControlerConfig)
    : _mutex(), _device(device), _id(ID),
      _controlerConfig(motorControlerConfig),
      _conversionFactor(calculateConversionFactor(motorControlerConfig)),
      _currentVmax(motorControlerConfig.maximumVelocity),
      _usrSetCurrentScale(
          _controlerConfig.stallGuardControlData.getCurrentScale()),
      ...
      _controlerStatus{device->getScalarRegisterAccessor<int32_t>(
          moduleName + "/" + CONTROLER_STATUS_BITS_ADDRESS_STRING, 0,
          {ChimeraTK::AccessMode::raw})},
      _actualPosition{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, ACTUAL_POSITION_SUFFIX)},
      _actualVelocity{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, ACTUAL_VELOCITY_SUFFIX)},
      _actualAcceleration{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, ACTUAL_ACCELETATION_SUFFIX)},
      _microStepCount{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, MICRO_STEP_COUNT_SUFFIX)},
      _stallGuardValue{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, STALL_GUARD_VALUE_SUFFIX)},
      _coolStepValue{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, COOL_STEP_VALUE_SUFFIX)},
      _status{RAW_ACCESSOR_FROM_SUFFIX(moduleName, STATUS_SUFFIX)},
      _motorCurrentEnabled{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, MOTOR_CURRENT_ENABLE_SUFFIX)},
      _decoderReadoutMode{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, DECODER_READOUT_MODE_SUFFIX)},
      _decoderPosition{
          RAW_ACCESSOR_FROM_SUFFIX(moduleName, DECODER_POSITION_SUFFIX)},
      _calibrationTime{device->getScalarRegisterAccessor<int32_t>(
          moduleName + "/" + CALIBRATION_TIME, 0,
          {ChimeraTK::AccessMode::raw})},
      ...
      _controlerSPI(controlerSPI), converter24bits(24), converter12bits(12),
      _moveOnlyFullStep(false), _userMicroStepSize(0), _localTargetPosition(0) {
  ...
  setDividersAndMicroStepResolutionData(
  ...
  setReferenceConfigAndRampModeData(
      motorControlerConfig.referenceConfigAndRampModeData);
  ...
  _userMicroStepSize =
      pow(2, motorControlerConfig.driverControlData.getMicroStepResolution());
  ...
  try {
    ...
    _endSwithPowerIndicator.replace(
        RAW_ACCESSOR_FROM_SUFFIX(moduleName, ENDSWITCH_ENABLE_SUFFIX));
...
unsigned int MotorControlerImpl::readRegisterAccessor(
    ChimeraTK::ScalarRegisterAccessor<int32_t> &readValue) {
  readValue.read();
...
int MotorControlerImpl::retrieveTargetPositonAndConvert() {
  int readValue =
      static_cast<int>(_controlerSPI->read(_id, IDX_TARGET_POSITION).getDATA());
...
void MotorControlerImpl::roundToNextFullStep(
    int &targetPosition) { // todo implementing
  ...
  unsigned int newActualMicroStepCount =
      (actualMicroStepCount + deltaMicroStep) & 0x3FF;
  unsigned int distanceToPreviousFullStep = (newActualMicroStepCount + 1) % 256;
  if (distanceToPreviousFullStep < 128) {
    targetPosition =
        targetPosition - distanceToPreviousFullStep / _userMicroStepSize;
  } else {
    targetPosition = targetPosition +
                     (256 - distanceToPreviousFullStep) / _userMicroStepSize;
  }
}

DEFINE_GET_SET_VALUE(MinimumVelocity, IDX_MINIMUM_VELOCITY)
DEFINE_SIGNED_GET_SET_VALUE(TargetVelocity, IDX_TARGET_VELOCITY,
                            converter12bits)
...
template <class T> T MotorControlerImpl::readTypedRegister() {
  T typedWord;
  typedWord.setSMDA(_id);
  TMC429OutputWord readbackWord =
      _controlerSPI->read(_id, typedWord.getIDX_JDX());
...
double MotorControlerImpl::setUserSpeedLimit(double microStepsPerSecond) {
  auto validatedVmax =
      validateVMaxForHardware(convertUstepsPerSecToVmax(microStepsPerSecond));
...
template <class T>
void MotorControlerImpl::setTypedDriverData(T const &driverData,
                                            T &localDataInstance) {
...
MotorReferenceSwitchData MotorControlerImpl::retrieveReferenceSwitchStatus() {
  ...
  unsigned int commonReferenceSwitchWord =
      _controlerSPI->read(SMDA_COMMON, JDX_REFERENCE_SWITCH).getDATA();
  unsigned int dataWord = (commonReferenceSwitchWord & bitMask) >> 2 * _id;
  // note: the following code uses the implicit bool conversion to/from 0/1 to
  // keep the code short.
  MotorReferenceSwitchData motorReferenceSwitchData(dataWord);

  auto configAndRampModeData =
      readTypedRegister<ReferenceConfigAndRampModeData>();
  motorReferenceSwitchData.setNegativeSwitchEnabled(
      !configAndRampModeData.getDISABLE_STOP_L());
  motorReferenceSwitchData.setPositiveSwitchEnabled(
      !configAndRampModeData.getDISABLE_STOP_R());
...
void MotorControlerImpl::setNegativeReferenceSwitchEnabled(bool enableStatus) {
  lock_guard guard(_mutex);
  auto referenceConfigAndRampModeData =
      readTypedRegister<ReferenceConfigAndRampModeData>();
...
void MotorControlerImpl::setPositiveReferenceSwitchCalibration(
...
void MotorControlerImpl::setNegativeReferenceSwitchCalibration(
    int calibratedPosition) {
  // TODO
  (void)calibratedPosition;
}
...
double
MotorControlerImpl::convertUstepsPerSecToVmax(double speedInUstepsPerSec) {
...
double MotorControlerImpl::calculateConversionFactor(
  ...
  auto pulse_div = static_cast<double>(
      motorControlerConfig.dividersAndMicroStepResolutionData
          .getPulseDivider());
...
double MotorControlerImpl::setUserCurrentLimit(double currentLimit) {
  auto currentScale =
      limitCurrentScale(convertAmpsToCurrentScale(currentLimit));
...
double MotorControlerImpl::getMaxCurrentLimit() {
  ...
  // object.
  auto configuredCurrentScale =
      _controlerConfig.stallGuardControlData.getCurrentScale();
  return convertCurrentScaletoAmps(configuredCurrentScale);
}

unsigned int
MotorControlerImpl::validateVMaxForHardware(double calculatedVmax) {
  ...
  if (calculatedVmax < minLimitForVmax) {
    return minLimitForVmax;
  }
...
double
MotorControlerImpl::convertCurrentScaletoAmps(unsigned int currentScale) {
...
double MotorControlerImpl::convertAmpsToCurrentScale(double currentInAmps) {
  auto currentScale =
      (currentInAmps / iMaxTMC260C_IN_AMPS * iMaxTMC260C_CURRENT_SCALE_VALUES) -
      1;
  return currentScale;
}

unsigned int
MotorControlerImpl::limitCurrentScale(double calculatedCurrentScale) {
  auto hardLimitForCurrentScale =
      _controlerConfig.stallGuardControlData.getCurrentScale();
  if (calculatedCurrentScale < iMaxTMC260C_MIN_CURRENT_SCALE_VALUE) {
    return iMaxTMC260C_MIN_CURRENT_SCALE_VALUE;
  }
  if (calculatedCurrentScale > hardLimitForCurrentScale) {
    return hardLimitForCurrentScale;
  }
  return (static_cast<unsigned int>(std::floor(
      calculatedCurrentScale))); // floor so that we dont exceed the set limt
}
...
bool MotorControlerImpl::isMotorMoving() {
  ...
  MotorReferenceSwitchData referenceSwitchData =
      retrieveReferenceSwitchStatus();
  if ((referenceSwitchData.getNegativeSwitchEnabled() &&
       referenceSwitchData.getNegativeSwitchActive() &&
       _localTargetPosition <= currentPos) ||
      (referenceSwitchData.getPositiveSwitchEnabled() &&
       referenceSwitchData.getPositiveSwitchActive() &&
       _localTargetPosition >= currentPos)) {
    DriverStatusData status(readRegisterAccessor(_status));
    return !status.getStandstillIndicator();
  }
