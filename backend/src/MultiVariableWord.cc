...
void MultiVariableWord::setSubWord(unsigned int subWord, unsigned int inputMask,
                                   unsigned char offset) {
  if (subWord & ~inputMask) {
    std::stringstream message;
    message << "Sub word is too large. Input mask is 0x" << std::hex
            << inputMask << ", sub word is 0x" << subWord << "=" << std::dec
            << subWord;
    throw MotorDriverException(message.str(),
                               MotorDriverException::OUT_OF_RANGE);
  }
  _dataWord =
      (_dataWord & ~(inputMask << offset)) | ((subWord & inputMask) << offset);
}
