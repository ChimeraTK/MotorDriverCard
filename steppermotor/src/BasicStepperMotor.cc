...
BasicStepperMotor::BasicStepperMotor(const StepperMotorParameters &parameters)
    : _motorDriverCard(
      ...
      _autostart(false), _softwareLimitsEnabled(false), _mutex(),
      _stateMachine(), _errorMode(Error::NO_ERROR),
...
BasicStepperMotor::BasicStepperMotor()
    : _motorDriverCard(), _motorControler(),
      ...
      _encoderUnitsConverter(
          std::make_shared<utility::EncoderStepsConverterTrivia>()),
      _encoderPositionOffset(0), _targetPositionInSteps(0),
      _maxPositionLimitInSteps(std::numeric_limits<int>::max()),
      _minPositionLimitInSteps(std::numeric_limits<int>::min()),
      _autostart(false), _softwareLimitsEnabled(false), _mutex(),
      _stateMachine(), _errorMode(Error::NO_ERROR),
...
bool BasicStepperMotor::limitsOK(int newPositionInSteps) {
  if (_softwareLimitsEnabled &&
      newPositionInSteps >= _minPositionLimitInSteps &&
...
ExitStatus BasicStepperMotor::setTargetPosition(float newPosition) {
  return setTargetPositionInSteps(
      _stepperMotorUnitsConverter->unitsToSteps(newPosition));
}
...
ExitStatus BasicStepperMotor::setMaxPositionLimit(float maxPosInUnits) {
  return setMaxPositionLimitInSteps(
      _stepperMotorUnitsConverter->unitsToSteps(maxPosInUnits));
}
...
ExitStatus BasicStepperMotor::setMinPositionLimit(float minPosInUnits) {
  return setMinPositionLimitInSteps(
      _stepperMotorUnitsConverter->unitsToSteps(minPosInUnits));
}
...
void BasicStepperMotor::resetMotorControllerPositions(
    int actualPositionInSteps) {
...
void BasicStepperMotor::setActualPositionActions(int actualPositionInSteps) {
  ...
  _motorControler->setPositiveReferenceSwitchCalibration(
      std::numeric_limits<int>::max());
  _motorControler->setNegativeReferenceSwitchCalibration(
...
ExitStatus
BasicStepperMotor::setActualPositionInSteps(int actualPositionInSteps) {
  LockGuard guard(_mutex);
  if (motorActive()) {
    return ExitStatus::ERR_SYSTEM_IN_ACTION;
  }
...
ExitStatus BasicStepperMotor::setActualPosition(float actualPosition) {
  return setActualPositionInSteps(
      _stepperMotorUnitsConverter->unitsToSteps(actualPosition));
}
...
ExitStatus BasicStepperMotor::translateAxis(float translationInUnits) {
  return translateAxisInSteps(
      _stepperMotorUnitsConverter->unitsToSteps(translationInUnits));
}
...
float BasicStepperMotor::getCurrentPosition() {
  LockGuard guard(_mutex);
  return _stepperMotorUnitsConverter->stepsToUnits(
...
double BasicStepperMotor::getEncoderPosition() {
  ...
  return _encoderUnitsConverter->stepsToUnits(
      ...
      _encoderPositionOffset);
}

ExitStatus
BasicStepperMotor::setActualEncoderPosition(double referencePosition) {
  LockGuard guard(_mutex);
  if (motorActive()) {
    return ExitStatus::ERR_SYSTEM_IN_ACTION;
  }
  ...
  _encoderPositionOffset =
      _encoderUnitsConverter->unitsToSteps(referencePosition) -
      static_cast<int>(_motorControler->getDecoderPosition());
  return ExitStatus::SUCCESS;
}
...
float BasicStepperMotor::getTargetPosition() {
  LockGuard guard(_mutex);
  return _stepperMotorUnitsConverter->stepsToUnits(
      _motorControler->getTargetPosition());
}
...
ExitStatus BasicStepperMotor::setStepperMotorUnitsConverterToDefault() {
  ...
  _stepperMotorUnitsConverter =
      std::make_unique<utility::MotorStepsConverterTrivia>();
  return ExitStatus::SUCCESS;
}
...
double BasicStepperMotor::getSafeCurrentLimit() {
  ...
  try {
    ...
    throw StepperMotorException(e.what(),
                                StepperMotorException::FEATURE_NOT_AVAILABLE);
  }
  return currentLimit;
}
...
ExitStatus BasicStepperMotor::calibrate() {
  throw StepperMotorException(
      "This routine is not available for the BasicStepperMotor",
      StepperMotorException::FEATURE_NOT_AVAILABLE);
}
