...
StepperMotorFactory::create(const StepperMotorParameters &parameters) {
  std::lock_guard<std::mutex> lck(_factoryMutex);

  // Check if instance already exists and
  // return it, if this is given
  MotorIdentifier id = std::make_tuple(
      parameters.deviceName, parameters.moduleName, parameters.driverId);
