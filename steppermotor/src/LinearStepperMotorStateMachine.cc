...
const StateMachine::Event
    LinearStepperMotor::StateMachine::calibEvent("calibEvent");
const StateMachine::Event
    LinearStepperMotor::StateMachine::calcToleranceEvent("calcToleranceEvent");

LinearStepperMotor::StateMachine::StateMachine(
    LinearStepperMotor &stepperMotorWithReference)
    : BasicStepperMotor::StateMachine(stepperMotorWithReference),
      ...
      _motor(stepperMotorWithReference), _stopAction(false),
      _moveInterrupted(false) {
  _idle.setTransition(
      calibEvent, &_calibrating,
  ...
  _idle.setTransition(
      LinearStepperMotor::StateMachine::calcToleranceEvent,
  ...
  _calibrating.setTransition(
      BasicStepperMotor::StateMachine::stopEvent, &_idle,
      std::bind(&LinearStepperMotor::StateMachine::actionStop, this));
  _calibrating.setTransition(
      BasicStepperMotor::StateMachine::emergencyStopEvent, &_error,
      [this] { actionEmergencyStop(); });
  _calibrating.setTransition(BasicStepperMotor::StateMachine::errorEvent,
                             &_error, [] {});
...
void LinearStepperMotor::StateMachine::actionStartCalib() {
  ...
  std::thread calibrationThread(
      &LinearStepperMotor::StateMachine::calibrationThreadFunction, this);
...
void LinearStepperMotor::StateMachine::actionStartCalcTolercance() {
  _asyncActionActive.exchange(true);
  std::thread toleranceCalcThread(
      &LinearStepperMotor::StateMachine::toleranceCalcThreadFunction, this);
...
void LinearStepperMotor::StateMachine::calibrationThreadFunction() {
  ...
  if (!(_motor._positiveEndSwitchEnabled.load() &&
        _motor._negativeEndSwitchEnabled.load())) {
    ...
    if (_moveInterrupted.load() || _stopAction.load()) {
      ...
      calibPositiveEndSwitchInSteps =
          calibPositiveEndSwitchInSteps - calibNegativeEndSwitchInSteps;
      ...
      _motor._calibPositiveEndSwitchInSteps.exchange(
          calibPositiveEndSwitchInSteps);
...
void LinearStepperMotor::StateMachine::findEndSwitch(Sign sign) {
  while (!_motor.isEndSwitchActive(sign)) {
    if (_stopAction.load() || _moveInterrupted.load()) {
      return;
    } else if (_motor.getTargetPositionInSteps() !=
                   _motor.getCurrentPositionInSteps() &&
...
void LinearStepperMotor::StateMachine::moveToEndSwitch(Sign sign) {

  {
    ...
    _motor._motorControler->setTargetPosition(
        _motor._motorControler->getActualPosition() +
        static_cast<int>(sign) * 50000);
  }
  while (_motor._motorControler->isMotorMoving()) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(wakeupPeriodInMilliseconds));
  }
}
...
void LinearStepperMotor::StateMachine::toleranceCalcThreadFunction() {
  ...
  if (_motor._calibrationMode != CalibrationMode::FULL) {
    ...
    _motor._tolerancePositiveEndSwitch.exchange(
        getToleranceEndSwitch(Sign::POSITIVE));
    _motor._toleranceNegativeEndSwitch.exchange(
        getToleranceEndSwitch(Sign::NEGATIVE));
...
double LinearStepperMotor::StateMachine::getToleranceEndSwitch(Sign sign) {
  ...
  for (unsigned i = 0; i < N_TOLERANCE_CALC_SAMPLES; i++) {
    ...
    {
      boost::lock_guard<boost::mutex> lck(_motor._mutex);
      _motor._motorControler->setTargetPosition(endSwitchPosition -
                                                static_cast<int>(sign) * 1000);
    }
    while (_motor._motorControler->isMotorMoving()) {
      std::this_thread::sleep_for(
          std::chrono::milliseconds(wakeupPeriodInMilliseconds));
  ...
  if (!(_stopAction.load() || _moveInterrupted.load())) {
    for (unsigned int i = 0; i < N_TOLERANCE_CALC_SAMPLES; i++) {
      stdMeasurement += ((measurements[i] - meanMeasurement) /
                         (N_TOLERANCE_CALC_SAMPLES - 1)) *
