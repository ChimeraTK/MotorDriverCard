...
LinearStepperMotor::LinearStepperMotor(const StepperMotorParameters &parameters)
    : BasicStepperMotor(), _positiveEndSwitchEnabled(false),
      _negativeEndSwitchEnabled(false), _calibrationFailed(false),
      ...
      _tolerancePositiveEndSwitch(0), _toleranceNegativeEndSwitch(0) {
  _motorDriverCard =
...
bool LinearStepperMotor::motorActive() {
  ...
  if (stateName == "moving" || stateName == "calibrating" ||
      stateName == "calculatingTolerance") {
    return true;
  } else {
    return false;
  }
...
ExitStatus
LinearStepperMotor::setActualPositionInSteps(int actualPositionInSteps) {
...
ExitStatus LinearStepperMotor::translateAxisInSteps(int translationInSteps) {
  ...
  if (this->_calibrationMode.load() == CalibrationMode::FULL) {
    if (checkIfOverflow(_calibPositiveEndSwitchInSteps.load(),
                        translationInSteps) ||
        checkIfOverflow(_calibNegativeEndSwitchInSteps.load(),
                        translationInSteps)) {
...
float LinearStepperMotor::getPositiveEndReference() {
  ...
  return _stepperMotorUnitsConverter->stepsToUnits(
      _calibPositiveEndSwitchInSteps.load());
}
...
float LinearStepperMotor::getNegativeEndReference() {
  LockGuard guard(_mutex);
  return _stepperMotorUnitsConverter->stepsToUnits(
      _calibNegativeEndSwitchInSteps.load());
}
...
void LinearStepperMotor::loadEndSwitchCalibration() {
  ...
  if (_motorControler->getCalibrationTime() != 0) {
    ...
    int calibPositiveEndSwitchInSteps =
        _motorControler->getPositiveReferenceSwitchCalibration();
    int calibNegativeEndSwitchInSteps =
        _motorControler->getNegativeReferenceSwitchCalibration();
...
bool LinearStepperMotor::verifyMoveAction() {
  ...
  bool endSwitchActive =
      _motorControler->getReferenceSwitchData().getPositiveSwitchActive() ||
      _motorControler->getReferenceSwitchData().getNegativeSwitchActive();
  bool positionMatch = _motorControler->getTargetPosition() ==
...
bool LinearStepperMotor::isEndSwitchActive(Sign sign) {
  bool posActive =
      _motorControler->getReferenceSwitchData().getPositiveSwitchActive() == 1U;
  bool negActive =
      _motorControler->getReferenceSwitchData().getNegativeSwitchActive() == 1U;
  ...
  if (_toleranceCalculated) {
    if (posActive) {
      if (std::abs(_motorControler->getActualPosition() -
                   _calibPositiveEndSwitchInSteps.load()) >
