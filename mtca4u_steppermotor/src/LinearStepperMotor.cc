...
LinearStepperMotor::LinearStepperMotor(
    std::string const &motorDriverCardDeviceName, std::string const &moduleName,
    unsigned int motorDriverId, std::string motorDriverCardConfigFileName)
    : StepperMotor(motorDriverCardDeviceName, moduleName, motorDriverId,
                   motorDriverCardConfigFileName),
      _calibNegativeEndSwitchInSteps(0), _calibPositiveEndSwitchInSteps(0),
      _stopMotorCalibration(false),
      _negativeEndSwitchEnabled(
          _motorControler->getReferenceSwitchData().getNegativeSwitchEnabled()),
      _positiveEndSwitchEnabled(_motorControler->getReferenceSwitchData()
                                    .getPositiveSwitchEnabled()) {}
...
LinearStepperMotor::moveToPositionInSteps(int newPositionInSteps) {
...
LinearStepperMotorStatusAndError
LinearStepperMotor::moveToPosition(float newPosition) {
...
StepperMotorCalibrationStatus LinearStepperMotor::calibrateMotor() {
  ...
  if (_motorControler->getReferenceSwitchData().getPositiveSwitchEnabled() ==
          false ||
      _motorControler->getReferenceSwitchData().getNegativeSwitchEnabled() ==
          false) {
    _logger(Logger::INFO)
        << "LinearStepperMotor::calibrateMotor : End switches not available. "
  ...
  // enable the motor
  _logger(Logger::INFO)
      << "LinearStepperMotor::calibrateMotor : Enable the motor driver."
      << std::endl;
  ...
  _logger(Logger::INFO) << "LinearStepperMotor::calibrateMotor : Negative end "
                        ...
                        << " Sending to positive end switch.\n";
  _logger(Logger::DETAIL)
      << "LinearStepperMotor::calibrateMotor : Current position is: "
      << _motorControler->getActualPosition()
      << ". Go to positive end switch.\n";
  ...
  _logger(Logger::INFO) << "LinearStepperMotor::calibrateMotor : Negative end "
                           "switch is at (steps):  "
                        << _calibNegativeEndSwitchInSteps
                        << " Positive end switch at (steps): "
                        << _calibPositiveEndSwitchInSteps << std::endl;
  //_logger(Logger::INFO) << "LinearStepperMotor::calibrateMotor : Negative end
  //switch is at (units):  " << _calibNegativeEndSwitchInUnits << " Positive end
  //switch at (units): " << _calibPositiveEndSwitchInUnits << std::endl;
  int newPos =
      (_calibPositiveEndSwitchInSteps + _calibNegativeEndSwitchInSteps) / 2;
  ...
  if (_stopMotorCalibration == true) {
    ...
    _logger(Logger::WARNING)
        << "LinearStepperMotor::calibrateMotor : StepperMotor::calibrateMotor "
           ": Motor stopped by user.\n"
        << std::flush;
    _motorCalibrationStatus =
        StepperMotorCalibrationStatusType::M_CALIBRATION_STOPPED_BY_USER;
  } else if (statusAndError.status ==
             mtca4u::LinearStepperMotorStatusTypes::M_ERROR) {
    _motorCalibrationStatus =
        StepperMotorCalibrationStatusType::M_CALIBRATION_FAILED;
  } else {
...
void LinearStepperMotor::setCurrentPositionInStepsAs(int newPositionInSteps) {
  if (_motorCalibrationStatus ==
    ...
    _logger(Logger::INFO) << "LinearStepperMotor::setCurrenPositionAs : Motor "
                          ...
                          << std::endl;
    _logger(Logger::INFO)
        << "LinearStepperMotor::setCurrenPositionAs : Current position: "
        << getCurrentPosition() << ", in steps: " << getCurrentPositionInSteps()
        << std::endl;
    _logger(Logger::INFO)
        << "LinearStepperMotor::setCurrenPositionAs : Wanted  position: "
        << recalculateStepsToUnits(newPositionInSteps)
        << ", in steps: " << newPositionInSteps << std::endl;
    _logger(Logger::INFO)
        << "LinearStepperMotor::setCurrenPositionAs : Delta   position: "
        << recalculateStepsToUnits(delta) << ", in steps: " << delta
        << std::endl;
    _logger(Logger::INFO) << "LinearStepperMotor::setCurrenPositionAs : "
                             "Negative end switch (in steps): "
                          << _calibNegativeEndSwitchInSteps
                          << " positive end switch(in steps): "
                          << _calibPositiveEndSwitchInSteps << std::endl;
...
LinearStepperMotorStatusAndError
LinearStepperMotor::determineMotorStatusAndError() {
  boost::lock_guard<boost::mutex> lock_guard(_mutex);
  ...
  if (positiveSwitchStatus && negativeSwitchStatus) {
    ...
    positiveSwitchStatus =
        _motorControler->getReferenceSwitchData().getPositiveSwitchActive();
    negativeSwitchStatus =
        _motorControler->getReferenceSwitchData().getNegativeSwitchActive();

    if (positiveSwitchStatus)
      _logger(Logger::ERROR)
          << "LinearStepperMotor::determineMotorStatusAndError(): Positive "
             "end-switch on. "
          << std::endl;

    if (negativeSwitchStatus)
      _logger(Logger::ERROR)
          << "LinearStepperMotor::determineMotorStatusAndError(): Negative "
             "end-switch on. "
          << std::endl;
    ...
    if (positiveSwitchStatus && negativeSwitchStatus) {
      ...
      _logger(Logger::ERROR)
          << "LinearStepperMotor::determineMotorStatusAndError(): Both end "
  ...
  if (_softwareLimitsEnabled &&
      _maxPositionLimitInSteps <= _minPositionLimitInSteps) {
    _motorStatus = LinearStepperMotorStatusTypes::M_ERROR;
    _motorError =
  ...
  if (_softwareLimitsEnabled) {
    ...
    if (_targetPositionInSteps <= _minPositionLimitInSteps) {
      _motorStatus =
          LinearStepperMotorStatusTypes::M_SOFT_NEGATIVE_END_SWITCHED_ON;
      return LinearStepperMotorStatusAndError(_motorStatus, _motorError);
    }
  }
...
void LinearStepperMotor::moveToEndSwitch(
    LinearStepperMotorStatus targetStatus) {
  LinearStepperMotorStatusAndError statusAndError;
  ...
  LinearStepperMotorStatus errortargetStatus =
  ...
  do {
    ...
    _logger(Logger::INFO)
        << "LinearStepperMotor::calibrateMotor : Current position (steps): "
        << currentPosition
        << " (units): " << recalculateStepsToUnits(currentPosition)
        << " New position (steps): " << currentPosition + dir * 500000
        << " (units): "
        << recalculateStepsToUnits(currentPosition + dir * 500000) << std::endl;
    ...
    if (_stopMotorCalibration == true) {
      _stopMotorCalibration = false;
      _logger(Logger::WARNING)
          << "LinearStepperMotor::calibrateMotor : Calibration stopped by user."
          << std::endl;
