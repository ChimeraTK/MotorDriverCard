...
const StepperMotorError
    StepperMotorErrorTypes::M_CONFIG_ERROR_MIN_POS_GRATER_EQUAL_TO_MAX(4);
...
const LinearStepperMotorError
    LinearStepperMotorErrorTypes::M_BOTH_END_SWITCH_ON(32);
...
StepperMotorError::StepperMotorError(const StepperMotorError &error)
    : GeneralStatus(error) {}
...
std::string StepperMotorError::asString() const {
  ...
  } else if (_id == StepperMotorErrorTypes::
...
LinearStepperMotorError::LinearStepperMotorError(int id)
    : StepperMotorError(id) {}

LinearStepperMotorError::LinearStepperMotorError(
    const LinearStepperMotorError &error)
