...
ParameterWidget::ParameterWidget(QWidget *parent_,
                                 std::string const &parameterName,
                                 uint32_t defaultValue)
    : QWidget(parent_), horizontalLayout(NULL), label(NULL),
      parameterSpinBox(NULL), defaultValueLabel(NULL) {
  ...
  defaultValueLabel =
      new QLabel(QString("0x%1 ").arg(defaultValue, 0, 16), this);
...
uint32_t ParameterWidget::getValue() { return parameterSpinBox->value(); }
