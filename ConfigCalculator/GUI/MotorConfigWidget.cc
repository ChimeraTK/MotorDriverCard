...
MotorConfigWidget::MotorConfigWidget(QWidget *parent_)
    : QWidget(parent_),
      ...
      _chipParameters(6,    // pulseDiv_
                      ...
                      3,    // currentScale_
                      std::list<std::string>()),
      _motorConfigWidgetForm(), _motorExpertPanel(NULL), _expertTabWidget(NULL),
      _tabIndex(0) {
  _motorConfigWidgetForm.setupUi(this);

  // recalculate all parameters whenever an input changes
  connect(_motorConfigWidgetForm.iMax, SIGNAL(valueChanged(double)), this,
          SLOT(recalculateChipParameters()));

  connect(_motorConfigWidgetForm.nStepsPerTurn, SIGNAL(valueChanged(int)), this,
          SLOT(recalculateChipParameters()));

  connect(_motorConfigWidgetForm.maxRPM, SIGNAL(valueChanged(int)), this,
          SLOT(recalculateChipParameters()));

  connect(_motorConfigWidgetForm.timeToVMax, SIGNAL(valueChanged(double)), this,
          SLOT(recalculateChipParameters()));

  connect(_motorConfigWidgetForm.microsteps, SIGNAL(currentIndexChanged(int)),
          this, SLOT(recalculateChipParameters()));

  connect(_motorConfigWidgetForm.systemClock, SIGNAL(valueChanged(int)), this,
          SLOT(recalculateChipParameters()));

  connect(_motorConfigWidgetForm.positiveSwitchCheckBox,
          SIGNAL(stateChanged(int)), this, SLOT(recalculateChipParameters()));

  connect(_motorConfigWidgetForm.negativeSwitchCheckBox,
          SIGNAL(stateChanged(int)), this, SLOT(recalculateChipParameters()));
...
void MotorConfigWidget::recalculateChipParameters() {
  unsigned int microsteps =
      pow(2, _motorConfigWidgetForm.microsteps->currentIndex());

  ParametersCalculator::PhysicalParameters physicalParameters(
      _motorConfigWidgetForm.systemClock->value(), microsteps,
...
void MotorConfigWidget::updateMotorExpertPanel() {
  ...
  mtca4u::MotorControlerConfig expertConfig =
      getMotorParameters(_motorExpertPanel);
  ...
  // to the expert.
  expertConfig.driverControlData.setMicroStepResolution(
      calculatedConfig.driverControlData.getMicroStepResolution());
  _motorExpertPanel->setParameter(
      "driverControlData", expertConfig.driverControlData.getPayloadData());
  expertConfig.stallGuardControlData.setCurrentScale(
      calculatedConfig.stallGuardControlData.getCurrentScale());
  _motorExpertPanel->setParameter(
      "stallGuardControlData",
      expertConfig.stallGuardControlData.getPayloadData());

  // the other parameters go directly through. We have to replace the whole word
  _motorExpertPanel->setParameter(
      "referenceConfigAndRampModeData",
      calculatedConfig.referenceConfigAndRampModeData.getDATA());
  _motorExpertPanel->setParameter(
      "proportionalityFactorData",
      calculatedConfig.proportionalityFactorData.getDATA());
  _motorExpertPanel->setParameter(
      "dividersAndMicroStepResolutionData",
      calculatedConfig.dividersAndMicroStepResolutionData.getDATA());
  _motorExpertPanel->setParameter("maximumVelocity",
                                  calculatedConfig.maximumVelocity);
  _motorExpertPanel->setParameter("maximumAcceleration",
                                  calculatedConfig.maximumAcceleration);
}
...
void MotorConfigWidget::setMotorExpertPanel(ParametersPanel *motorExpertPanel,
  ...
  setMotorExpertTabEnabled(
      _motorConfigWidgetForm.motorEnabledCheckBox->isChecked());
}
