...
ParametersPanel::ParametersPanel(QWidget *parent_)
    : QWidget(parent_), parametersMap(), verticalLayout(NULL) {
  ...
  QLabel *asteriskLabel = new QLabel(
      "Fields marked with (*) are modified by the basic config calculation.",
      this);
...
void ParametersPanel::addParameter(std::string const &parameterName,
  ...
  parametersMap[parameterName] = new ParameterWidget(
      this, parameterName + " " + additionalText, defaultValue);

  verticalLayout->addWidget(parametersMap[parameterName]);
}

uint32_t ParametersPanel::operator[](std::string const &parameterName) {
  std::map<std::string, ParameterWidget *>::const_iterator paramIter =
      parametersMap.find(parameterName);
...
void ParametersPanel::setParameter(std::string const &parameterName,
                                   uint32_t value) {
  std::map<std::string, ParameterWidget *>::const_iterator paramIter =
      parametersMap.find(parameterName);
  if (paramIter == parametersMap.end()) {
