...
int main(int argc, char *argv[]) {
  if ((argc < 5) || (argc > 8)) {
    std::cout
        << "Calculate the MD22 config parameters from the "
  ...
  std::cout << argv[1] << " " << iMax << " " << nStepsPerTurn << " " << maxRPM
            << " " << timeToVMax << " " << std::hex << "0x" << endSwitchConfig
            << std::dec << " " << microsteps << std::endl;

  ParametersCalculator::PhysicalParameters physicalParameters(
      32, // system clock, hard coded
