...
#pragma GCC diagnostic pop
  ...
  std::cout << std::hex << ".proportionalityFactorData 0x"
            << controllerConfig.proportionalityFactorData.getDATA() << std::dec
            << std::endl;
...
#pragma GCC diagnostic pop
  ...
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(
      chipParameters, ConfigCalculator::IGNORE_BOTH));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(
      chipParameters, ConfigCalculator::IGNORE_POSITIVE));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(
      chipParameters, ConfigCalculator::IGNORE_NEGATIVE));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(
      chipParameters, ConfigCalculator::IGNORE_BOTH));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(
      chipParameters, ConfigCalculator::USE_BOTH));

  BOOST_CHECK_THROW(
      ConfigCalculator::calculateConfig(
          chipParameters, static_cast<ConfigCalculator::EndSwitchConfig>(5)),
