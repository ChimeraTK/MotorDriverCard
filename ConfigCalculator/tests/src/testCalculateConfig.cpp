...
#pragma GCC diagnostic pop
  ...
  ParametersCalculator::ChipParameters chipParameters(1, // pulseDiv
      2,                                                 // rampDiv
      3,                                                 // aMax
      4,                                                 // vMax
      5,                                                 // pDiv
  ...
  BOOST_CHECK(controllerConfig.stallGuardControlData.getPayloadData() == 0x10009);
  BOOST_CHECK(controllerConfig.referenceConfigAndRampModeData.getDATA() == ConfigCalculator::IGNORE_BOTH);
  // proportionalityFactorData is 0xXX0Y with XX = pmul and y= pdiv
  BOOST_CHECK(controllerConfig.proportionalityFactorData.getDATA() == 0x8605);
  std::cout << std::hex << ".proportionalityFactorData 0x" << controllerConfig.proportionalityFactorData.getDATA()
            << std::dec << std::endl;
  // dividersAndMicroStepResolutionData is 0xXY0Z with
  // X=PulseDiv, Y=RampDiv, Z=controllerMicroStepValue
  BOOST_CHECK(controllerConfig.dividersAndMicroStepResolutionData.getDATA() == 0x1207);
...
#pragma GCC diagnostic pop
  ...
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(chipParameters, ConfigCalculator::IGNORE_BOTH));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(chipParameters, ConfigCalculator::IGNORE_POSITIVE));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(chipParameters, ConfigCalculator::IGNORE_NEGATIVE));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(chipParameters, ConfigCalculator::IGNORE_BOTH));
  BOOST_CHECK_NO_THROW(ConfigCalculator::calculateConfig(chipParameters, ConfigCalculator::USE_BOTH));

  BOOST_CHECK_THROW(
      ConfigCalculator::calculateConfig(chipParameters, static_cast<ConfigCalculator::EndSwitchConfig>(5)),
