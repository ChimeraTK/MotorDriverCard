...
bool operator<(const StateMachine::Event &event1,
               const StateMachine::Event &event2) {
  return event1._eventName < event2._eventName;
}

StateMachine::State::TransitionData::TransitionData(
    State *target, std::function<void(void)> entryCallback,
    std::function<void(void)> internalCallback)
    : targetState(target), entryCallbackAction(entryCallback),
      internalCallbackAction(internalCallback) {}

StateMachine::State::TransitionData::TransitionData(
    const State::TransitionData &targetAndAction)
...
StateMachine::State::State(std::string stateName)
    : _stateName(stateName), _transitionTable() {}

StateMachine::State::~State() {}

void StateMachine::State::setTransition(
    Event event, State *target, std::function<void(void)> entryCallback,
    std::function<void(void)> internalCallback) {
  TransitionData transitionData(target, entryCallback, internalCallback);
  _transitionTable.insert(
      std::pair<Event, TransitionData>(event, transitionData));
}

const StateMachine::State::TransitionTable &
StateMachine::State::getTransitionTable() const {
...
StateMachine::StateMachine()
    : _initState("initState"), _endState("endState"),
      _currentState(&_initState), _requestedState(nullptr),
      _stateMachineMutex(), _asyncActionActive(false),
...
void StateMachine::performTransition(Event event) {
  ...
  const State::TransitionTable &transitionTable =
      _currentState->getTransitionTable();
