...
public:
  MotorDriverCardDummyTestSuite()
      : test_suite("MotorDriverCardDummy test suite") {
    // create an instance of the test class
    boost::shared_ptr<MotorDriverCardDummyTest> motorDriverCardTest(
        new MotorDriverCardDummyTest);
...
void MotorDriverCardDummyTest::testGetMotorControler() {
  boost::shared_ptr<MotorControler> controler0 =
      _motorDriverCardDummy.getMotorControler(0);
  ...
  boost::shared_ptr<MotorControlerDummy> controler1 =
      boost::dynamic_pointer_cast<MotorControlerDummy>(
          _motorDriverCardDummy.getMotorControler(1));

  // this will cause an exception if the cast failed
  BOOST_CHECK(controler1->getID() == 1);

  BOOST_CHECK_THROW(_motorDriverCardDummy.getMotorControler(2),
                    MotorDriverException);
}

void MotorDriverCardDummyTest::testGetPowerMonitor() {
  BOOST_CHECK_THROW(_motorDriverCardDummy.getPowerMonitor(),
                    MotorDriverException);
}
...
test_suite *init_unit_test_suite(int /*argc*/, char * /*argv*/ []) {
  framework::master_test_suite().p_name.value =
      "MotorDriverCardDummy test suite";
