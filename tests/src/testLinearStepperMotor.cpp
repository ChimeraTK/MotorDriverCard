...
class LinearStepperMotorTestSuite : public test_suite {
 public:
  LinearStepperMotorTestSuite() : test_suite("LinearStepperMotor suite") {
    ...
    add(BOOST_CLASS_TEST_CASE(&LinearStepperMotorTest::testSoftLimits, stepperMotorTest));

    add(BOOST_CLASS_TEST_CASE(&LinearStepperMotorTest::testStop, stepperMotorTest));

    add(BOOST_CLASS_TEST_CASE(&LinearStepperMotorTest::testEmergencyStop, stepperMotorTest));

    add(BOOST_CLASS_TEST_CASE(&LinearStepperMotorTest::testSetPositionAs, stepperMotorTest));

    add(BOOST_CLASS_TEST_CASE(&LinearStepperMotorTest::testMoveToPosition, stepperMotorTest));

    add(BOOST_CLASS_TEST_CASE(&LinearStepperMotorTest::testCalibration, stepperMotorTest));

    add(BOOST_CLASS_TEST_CASE(&LinearStepperMotorTest::testIsMoving, stepperMotorTest));
  }
};
...
void LinearStepperMotorTest::testMoveToPosition() {
  ...
  BOOST_CHECK(statusAndError.error == StepperMotorErrorTypes::M_NO_ERROR);
  BOOST_CHECK(_motorControlerDummy->getTargetPosition() == (currentPosition - 100) * 10);
  _stepperMotor->setSoftwareLimitsEnabled(false);
...
void LinearStepperMotorTest::threadMoveToPostion(int testCase) {
  if(testCase == 1) {
    while(_stepperMotor->isMoving() == false) { // the parent thread has not called moveToPosition yet.
      usleep(1000);
    }
  ...
  if(testCase == 3) {
    while(_motorControlerDummy->getReferenceSwitchData().getPositiveSwitchActive() == false) {
      usleep(1000);
    }
    _motorControlerDummy->moveTowardsTarget(1);
  }
  ...
  if(testCase == 4) {
    while(_motorControlerDummy->getReferenceSwitchData().getNegativeSwitchActive() == false) {
      usleep(1000);
    }
    _motorControlerDummy->moveTowardsTarget(1);
  }
...
void LinearStepperMotorTest::testCalibration() {
  ...
  _motorControlerDummy->moveTowardsTarget(0.1, false, true); // in this case both end switches should get active in
                                                             // the same time
  calibrationStatus = _stepperMotor->calibrateMotor();
  BOOST_CHECK(calibrationStatus == StepperMotorCalibrationStatusType::M_CALIBRATION_FAILED);

  // test stop calibration
  _motorControlerDummy->resetInternalStateToDefaults(); // just to reset flags - _blockMotor and
  ...
  BOOST_CHECK(_stepperMotor->getNegativeEndSwitchPositionInSteps() == -10000 + 500);
  BOOST_CHECK(_stepperMotor->getPositiveEndSwitchPositionInSteps() == 10000 + 500);
...
bool LinearStepperMotorTest::isMoveToNegativeEndSwitchRequested() {
  return _motorControlerDummy->getTargetPosition() <= _motorControlerDummy->getNEgativeEndSwitch();
}

bool LinearStepperMotorTest::isMoveToPositiveEndSwitchRequested() {
  return _motorControlerDummy->getTargetPosition() >= _motorControlerDummy->getPositiveEndSwitch();
}

bool LinearStepperMotorTest::isMoveToValidPositionRequested() {
  return (_motorControlerDummy->getTargetPosition() < _motorControlerDummy->getPositiveEndSwitch()) &&
      (_motorControlerDummy->getTargetPosition() > _motorControlerDummy->getNEgativeEndSwitch());
}
