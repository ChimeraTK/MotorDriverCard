...
#define ADD_GET_SET_TEST(NAME)                                                 \
  test_case *set##NAME##TestCase = BOOST_CLASS_TEST_CASE(                      \
      &MotorDriverCardTest::testSet##NAME, motorDriverCardTest);               \
  test_case *get##NAME##TestCase = BOOST_CLASS_TEST_CASE(                      \
      &MotorDriverCardTest::testGet##NAME, motorDriverCardTest);               \
...
public:
  MotorDriverCardTestSuite() : test_suite(" MotorDriverCard test suite") {
    ...
    boost::shared_ptr<MotorDriverCardTest> motorDriverCardTest(
        new MotorDriverCardTest(MAP_FILE_NAME, MODULE_NAME_0));

    test_case *constructorTestCase = BOOST_CLASS_TEST_CASE(
        &MotorDriverCardTest::testConstructor, motorDriverCardTest);
    test_case *getControlerVersionTestCase = BOOST_CLASS_TEST_CASE(
        &MotorDriverCardTest::testGetControlerChipVersion, motorDriverCardTest);

    getControlerVersionTestCase->depends_on(constructorTestCase);

    add(constructorTestCase);
    add(getControlerVersionTestCase);
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardTest::testGetReferenceSwitchData,
                              motorDriverCardTest));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardTest::testGetStatusWord,
                              motorDriverCardTest));
    ...
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardTest::testPowerDown,
                              motorDriverCardTest));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardTest::testGetMotorControler,
                              motorDriverCardTest));
  }
};
...
MotorDriverCardTest::MotorDriverCardTest(std::string const &mapFileName,
                                         std::string const &moduleName)
    : _motorDriverCard(), _dummyDevice(), _registerMapping(),
      _mapFileName(mapFileName), _moduleName(moduleName) {}

void MotorDriverCardTest::testConstructor() {

  _dummyDevice = boost::dynamic_pointer_cast<DFMC_MD22Dummy>(
  ...
  motorDriverCardConfig.positionCompareInterruptData.setDATA(
      asciiToInt("PCID"));
  motorDriverCardConfig.positionCompareWord = asciiToInt("POSC");
  motorDriverCardConfig.stepperMotorGlobalParameters.setDATA(
      asciiToInt("SMGP"));

  for (unsigned int motorID = 0;
       motorID < motorDriverCardConfig.motorControlerConfigurations.size();
       ++motorID) {
    MotorControlerConfig motorControlerConfig;

    motorControlerConfig.accelerationThresholdData.setDATA(asciiToInt("THR") +
                                                           motorID);
    // motorControlerConfig.actualPosition = asciiToInt("ACX") + motorID;;
    motorControlerConfig.chopperControlData.setPayloadData(asciiToInt("cp") +
                                                           motorID);
    motorControlerConfig.coolStepControlData.setPayloadData(asciiToInt("cp") +
                                                            motorID);
    motorControlerConfig.decoderReadoutMode = asciiToInt("DRM") + motorID;
    motorControlerConfig.dividersAndMicroStepResolutionData.setDATA(
        asciiToInt("MUSR") + motorID);
    motorControlerConfig.driverConfigData.setPayloadData(asciiToInt("cf") +
                                                         motorID);
    motorControlerConfig.driverControlData.setPayloadData(asciiToInt("cl") +
                                                          motorID);
    ...
    motorControlerConfig.proportionalityFactorData.setDATA(asciiToInt("PROP") +
                                                           motorID);
    motorControlerConfig.referenceConfigAndRampModeData.setDATA(
        asciiToInt("REF") + motorID);
    motorControlerConfig.stallGuardControlData.setPayloadData(asciiToInt("sg") +
                                                              motorID);
    motorControlerConfig.targetPosition = asciiToInt("tx") + motorID;
    motorControlerConfig.targetVelocity = asciiToInt("tv") + motorID;

    motorDriverCardConfig.motorControlerConfigurations[motorID] =
        motorControlerConfig;
  }

  // has to throw because the device is not open
  BOOST_CHECK_THROW(
      _motorDriverCard = boost::shared_ptr<MotorDriverCardImpl>(
  ...
  boost::shared_ptr<mtca4u::DeviceBackend> brokenDummyDevice =
      boost::dynamic_pointer_cast<mtca4u::DeviceBackend>(
          ChimeraTK::BackendFactory::getInstance().createBackend(
              BROKEN_DUMMY_DEV_ALIAS));
  ...
  _dummyDevice->setFirmwareVersion(dfmc_md22::MINIMAL_FIRMWARE_VERSION +
                                   0x1000000);
  BOOST_CHECK_THROW(_motorDriverCard.reset(new MotorDriverCardImpl(
                        device, _moduleName, motorDriverCardConfig)),
...
void MotorDriverCardTest::testConfiguration(
  ...
  BOOST_CHECK(_motorDriverCard->getCoverDatagram() ==
              motorDriverCardConfig.coverDatagram);
  BOOST_CHECK(_motorDriverCard->getCoverPositionAndLength() ==
              motorDriverCardConfig.coverPositionAndLength);
  BOOST_CHECK(_motorDriverCard->getDatagramHighWord() ==
              motorDriverCardConfig.datagramHighWord);
  BOOST_CHECK(_motorDriverCard->getDatagramLowWord() ==
              motorDriverCardConfig.datagramLowWord);
  BOOST_CHECK(_motorDriverCard->getInterfaceConfiguration() ==
              motorDriverCardConfig.interfaceConfiguration);
  BOOST_CHECK(_motorDriverCard->getPositionCompareInterruptData() ==
              motorDriverCardConfig.positionCompareInterruptData);
  BOOST_CHECK(_motorDriverCard->getPositionCompareWord() ==
              motorDriverCardConfig.positionCompareWord);
  BOOST_CHECK(_motorDriverCard->getStepperMotorGlobalParameters() ==
              motorDriverCardConfig.stepperMotorGlobalParameters);

  for (unsigned int motorID = 0;
       motorID < motorDriverCardConfig.motorControlerConfigurations.size();
       ++motorID) {
    boost::shared_ptr<MotorControlerExpert> motorControler =
        boost::dynamic_pointer_cast<MotorControlerExpert>(
            _motorDriverCard->getMotorControler(motorID));
    MotorControlerConfig motorControlerConfig =
        motorDriverCardConfig.motorControlerConfigurations[motorID];
    ...
    BOOST_CHECK(motorControlerConfig.chopperControlData ==
                motorControler->getChopperControlData());
    BOOST_CHECK(motorControlerConfig.coolStepControlData ==
                motorControler->getCoolStepControlData());
    BOOST_CHECK(motorControlerConfig.decoderReadoutMode ==
                motorControler->getDecoderReadoutMode());
    BOOST_CHECK(
        motorControlerConfig.dividersAndMicroStepResolutionData.getDATA() ==
        motorControler->getDividersAndMicroStepResolutionData().getDATA());
    BOOST_CHECK(motorControlerConfig.driverConfigData ==
                motorControler->getDriverConfigData());
    BOOST_CHECK(motorControlerConfig.driverControlData ==
    ...
    BOOST_CHECK(motorControlerConfig.maximumAcceleration ==
                motorControler->getMaximumAcceleration());
    BOOST_CHECK(motorControlerConfig.maximumVelocity ==
                motorControler->getMaximumVelocity());
    BOOST_CHECK(motorControlerConfig.microStepCount ==
                motorControler->getMicroStepCount());
    BOOST_CHECK(motorControlerConfig.minimumVelocity ==
                motorControler->getMinimumVelocity());
    BOOST_CHECK(motorControlerConfig.positionTolerance ==
                motorControler->getPositionTolerance());
    ...
    BOOST_CHECK(motorControlerConfig.stallGuardControlData ==
                motorControler->getStallGuardControlData());
    // BOOST_CHECK( motorControlerConfig.targetPosition ==
    // motorControler->getTargetPosition() );
    BOOST_CHECK(motorControlerConfig.targetVelocity ==
                motorControler->getTargetVelocity());
  }
}

void MotorDriverCardTest::testGetControlerChipVersion() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_CHIP_VERSION);
...
void MotorDriverCardTest::testGetReferenceSwitchData() {
  ...
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_REFERENCE_SWITCH);
  BOOST_CHECK(_motorDriverCard->getReferenceSwitchData() ==
              ReferenceSwitchData(expectedContent));
}
...
void MotorDriverCardTest::testGetStatusWord() {
  ...
  _registerMapping->getRegisterInfo(CONTROLER_STATUS_BITS_ADDRESS_STRING,
                                    registerInfo, _moduleName);

  unsigned int expectedContent = testWordFromPCIeAddress(registerInfo.address);

  BOOST_CHECK(_motorDriverCard->getStatusWord().getDataWord() ==
              expectedContent);
}

void MotorDriverCardTest::testGetDatagramLowWord() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_DATAGRAM_LOW_WORD);
...
void MotorDriverCardTest::testGetDatagramHighWord() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_DATAGRAM_HIGH_WORD);
...
void MotorDriverCardTest::testGetCoverPositionAndLength() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_COVER_POSITION_AND_LENGTH);
  BOOST_CHECK(_motorDriverCard->getCoverPositionAndLength() ==
              CoverPositionAndLength(expectedContent));
}

void MotorDriverCardTest::testSetCoverPositionAndLength() {
  _motorDriverCard->setCoverPositionAndLength(0xFFFFFF);
  BOOST_CHECK(_motorDriverCard->getCoverPositionAndLength() ==
              CoverPositionAndLength(0xFFFFFF));
}

void MotorDriverCardTest::testGetCoverDatagram() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_COVER_DATAGRAM);
...
void MotorDriverCardTest::testGetStepperMotorGlobalParameters() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_STEPPER_MOTOR_GLOBAL_PARAMETERS);
  BOOST_CHECK(_motorDriverCard->getStepperMotorGlobalParameters() ==
              StepperMotorGlobalParameters(expectedContent));
}

void MotorDriverCardTest::testSetStepperMotorGlobalParameters() {
  _motorDriverCard->setStepperMotorGlobalParameters(0x555555);
  BOOST_CHECK(_motorDriverCard->getStepperMotorGlobalParameters() ==
              StepperMotorGlobalParameters(0x555555));
}

void MotorDriverCardTest::testGetInterfaceConfiguration() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_INTERFACE_CONFIGURATION);
  BOOST_CHECK(_motorDriverCard->getInterfaceConfiguration() ==
              InterfaceConfiguration(expectedContent));
}

void MotorDriverCardTest::testSetInterfaceConfiguration() {
  _motorDriverCard->setInterfaceConfiguration(0x555555);
  BOOST_CHECK(_motorDriverCard->getInterfaceConfiguration() ==
              InterfaceConfiguration(0x555555));
}

void MotorDriverCardTest::testGetPositionCompareWord() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_POSITION_COMPARE);
...
void MotorDriverCardTest::testGetPositionCompareInterruptData() {
  unsigned int expectedContent =
      testWordFromSpiAddress(SMDA_COMMON, JDX_POSITION_COMPARE_INTERRUPT);
  BOOST_CHECK(_motorDriverCard->getPositionCompareInterruptData() ==
              PositionCompareInterruptData(expectedContent));
}

void MotorDriverCardTest::testSetPositionCompareInterruptData() {
  _motorDriverCard->setPositionCompareInterruptData(0xAAAAAA);
  BOOST_CHECK(_motorDriverCard->getPositionCompareInterruptData() ==
              PositionCompareInterruptData(0xAAAAAA));
}
...
unsigned int MotorDriverCardTest::asciiToInt(std::string text) {
  ...
  for (/* emtpy */; (position < 4) && (characterIter != text.rend());
       ++position, ++characterIter) {
