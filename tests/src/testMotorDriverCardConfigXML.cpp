...
static const std::string
    COMPLETE_XML_FILE_NAME("MotorDriverCardConfig_complete_test.xml");
static const std::string
    FIRST_HALF_XML_FILE_NAME("MotorDriverCardConfig_first_half_test.xml");
static const std::string
    SECOND_HALF_XML_FILE_NAME("MotorDriverCardConfig_second_half_test.xml");
static const std::string
    MINIMAL_XML_FILE_NAME("MotorDriverCardConfig_minimal_test.xml");
static const std::string
    OLD_INVALID_XML_PREFIX("MotorDriverCardConfig_invalid_");
static const std::string OLD_INVALID_XML_SUFFIX(".xml");

static const std::string WRITE_FULL_COMPLETE_OUTPUT_FILE_NAME(
    "MotorDriverCardConfig_complete_full_output.xml");
static const std::string WRITE_SPARSE_COMPLETE_OUTPUT_FILE_NAME(
    "MotorDriverCardConfig_complete_sparse_output.xml");
static const std::string WRITE_SPARSE_FIRST_HALF_OUTPUT_FILE_NAME(
    "MotorDriverCardConfig_first_half_sparse_output.xml");
static const std::string WRITE_FULL_SECOND_HALF_OUTPUT_FILE_NAME(
    "MotorDriverCardConfig_second_half_full_output.xml");
static const std::string
    WRITE_MINIMAL_OUTPUT_FILE_NAME("MotorDriverCardConfig_minimal_output.xml");

#define ASSIGN_PLUS_ONE_CONTROLER(VARIABLE)                                    \
  plusOneControlerConfig.VARIABLE.setDataWord(                                 \
      defaultControlerConfig.VARIABLE.getDataWord() + 1)
...
#define CONTROLER_CHECK(VARIABLE)                                              \
  BOOST_CHECK_MESSAGE(inputControlerConfig.VARIABLE ==                         \
                          plusOneControlerConfig.VARIABLE,                     \
...
private:
  ...
  MotorDriverCardConfig
      completeCardConfig; ///< all values differ from the default
  MotorDriverCardConfig
      firstHalfCardConfig; ///< first half differs from default
  MotorDriverCardConfig
      secondHalfCardConfig; ///< second half differs from default

  MotorControlerConfig defaultControlerConfig;
  MotorControlerConfig
      plusOneControlerConfig; // all values are default + 1, true <-> false,
...
public:
  MotorDriverCardConfigXMLTestSuite()
      : test_suite(" MotorDriverCardConfigXML test suite") {
    // create an instance of the test class
    boost::shared_ptr<MotorDriverCardConfigXMLTest> xmlTest(
        new MotorDriverCardConfigXMLTest);

    add(BOOST_TEST_CASE(&MotorDriverCardConfigXMLTest::testInvalidInput));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardConfigXMLTest::testReadMinimal,
                              xmlTest));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardConfigXMLTest::testReadComplete,
                              xmlTest));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardConfigXMLTest::testReadFirstHalf,
                              xmlTest));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardConfigXMLTest::testReadSecondHalf,
                              xmlTest));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardConfigXMLTest::testWrite,
                              xmlTest));
    add(BOOST_CLASS_TEST_CASE(&MotorDriverCardConfigXMLTest::testInvalidOutput,
                              xmlTest));
    add(BOOST_TEST_CASE(boost::bind(
        &MotorDriverCardConfigXMLTest::testOldInvalidRegister, xmlTest,
...
test_suite *init_unit_test_suite(int argc, char *argv[]) {
  if (argc != 1) {
    std::cerr << argv[0] << " does not accept any arguments. They are ignored."
              << std::endl;
  }

  framework::master_test_suite().p_name.value =
      "MotorDriverCardConfigXML test suite";
  return new MotorDriverCardConfigXMLTestSuite;
}

MotorDriverCardConfigXMLTest::MotorDriverCardConfigXMLTest()
    : defaultCardConfig(), completeCardConfig(), firstHalfCardConfig(),
      secondHalfCardConfig(), defaultControlerConfig(),
  ...
  plusOneControlerConfig.targetPosition =
      -(plusOneControlerConfig.targetPosition + 1);
  plusOneControlerConfig.targetVelocity =
      -(plusOneControlerConfig.targetVelocity + 1);
...
void MotorDriverCardConfigXMLTest::testReadMinimal() {
  MotorDriverCardConfig inputCardConfig =
      MotorDriverCardConfigXML::read(MINIMAL_XML_FILE_NAME);
  BOOST_CHECK(inputCardConfig == defaultCardConfig);
}

void MotorDriverCardConfigXMLTest::testReadComplete() {
  MotorDriverCardConfig inputCardConfig =
      MotorDriverCardConfigXML::read(COMPLETE_XML_FILE_NAME);
  BOOST_CHECK(inputCardConfig.coverDatagram ==
              completeCardConfig.coverDatagram);
  BOOST_CHECK(inputCardConfig.coverPositionAndLength ==
              completeCardConfig.coverPositionAndLength);
  BOOST_CHECK(inputCardConfig.datagramHighWord ==
              completeCardConfig.datagramHighWord);
  BOOST_CHECK(inputCardConfig.datagramLowWord ==
              completeCardConfig.datagramLowWord);
  BOOST_CHECK(inputCardConfig.interfaceConfiguration ==
              completeCardConfig.interfaceConfiguration);
  BOOST_CHECK(inputCardConfig.positionCompareInterruptData ==
              completeCardConfig.positionCompareInterruptData);
  BOOST_CHECK(inputCardConfig.positionCompareWord ==
              completeCardConfig.positionCompareWord);
  BOOST_CHECK(inputCardConfig.stepperMotorGlobalParameters ==
              completeCardConfig.stepperMotorGlobalParameters);
  BOOST_CHECK(inputCardConfig.controlerSpiWaitingTime ==
              completeCardConfig.controlerSpiWaitingTime);

  for (size_t i = 0; i < inputCardConfig.motorControlerConfigurations.size();
       ++i) {
...
void MotorDriverCardConfigXMLTest::checkControlerConfig(
  ...
  message << "check in motorControlerConfig " << motorID
          << " failed for variable ";
...
void MotorDriverCardConfigXMLTest::testReadFirstHalf() {
  MotorDriverCardConfig inputCardConfig =
      MotorDriverCardConfigXML::read(FIRST_HALF_XML_FILE_NAME);
  BOOST_CHECK(inputCardConfig.coverDatagram ==
              completeCardConfig.coverDatagram);
  BOOST_CHECK(inputCardConfig.coverPositionAndLength ==
              defaultCardConfig.coverPositionAndLength);
  BOOST_CHECK(inputCardConfig.datagramHighWord ==
              completeCardConfig.datagramHighWord);
  BOOST_CHECK(inputCardConfig.datagramLowWord ==
              defaultCardConfig.datagramLowWord);
  BOOST_CHECK(inputCardConfig.interfaceConfiguration ==
              completeCardConfig.interfaceConfiguration);
  BOOST_CHECK(inputCardConfig.positionCompareInterruptData ==
              defaultCardConfig.positionCompareInterruptData);
  BOOST_CHECK(inputCardConfig.positionCompareWord ==
              completeCardConfig.positionCompareWord);
  BOOST_CHECK(inputCardConfig.stepperMotorGlobalParameters ==
              defaultCardConfig.stepperMotorGlobalParameters);
  BOOST_CHECK(inputCardConfig.controlerSpiWaitingTime ==
              completeCardConfig.controlerSpiWaitingTime);

  checkControlerConfig(inputCardConfig.motorControlerConfigurations[0], 0);
  BOOST_CHECK(inputCardConfig.motorControlerConfigurations[1] ==
              defaultControlerConfig);
}

void MotorDriverCardConfigXMLTest::testReadSecondHalf() {
  MotorDriverCardConfig inputCardConfig =
      MotorDriverCardConfigXML::read(SECOND_HALF_XML_FILE_NAME);
  BOOST_CHECK(inputCardConfig.coverDatagram == defaultCardConfig.coverDatagram);
  BOOST_CHECK(inputCardConfig.coverPositionAndLength ==
              completeCardConfig.coverPositionAndLength);
  BOOST_CHECK(inputCardConfig.datagramHighWord ==
              defaultCardConfig.datagramHighWord);
  BOOST_CHECK(inputCardConfig.datagramLowWord ==
              completeCardConfig.datagramLowWord);
  BOOST_CHECK(inputCardConfig.interfaceConfiguration ==
              defaultCardConfig.interfaceConfiguration);
  BOOST_CHECK(inputCardConfig.positionCompareInterruptData ==
              completeCardConfig.positionCompareInterruptData);
  BOOST_CHECK(inputCardConfig.positionCompareWord ==
              defaultCardConfig.positionCompareWord);
  BOOST_CHECK(inputCardConfig.stepperMotorGlobalParameters ==
              completeCardConfig.stepperMotorGlobalParameters);
  BOOST_CHECK(inputCardConfig.controlerSpiWaitingTime ==
              defaultCardConfig.controlerSpiWaitingTime);

  BOOST_CHECK(inputCardConfig.motorControlerConfigurations[0] ==
              defaultControlerConfig);
  checkControlerConfig(inputCardConfig.motorControlerConfigurations[1], 1);
}

void MotorDriverCardConfigXMLTest::testWrite() {
  MotorDriverCardConfigXML::write(WRITE_FULL_COMPLETE_OUTPUT_FILE_NAME,
                                  completeCardConfig);
  MotorDriverCardConfigXML::writeSparse(WRITE_SPARSE_COMPLETE_OUTPUT_FILE_NAME,
                                        completeCardConfig);
  MotorDriverCardConfigXML::writeSparse(
      WRITE_SPARSE_FIRST_HALF_OUTPUT_FILE_NAME, firstHalfCardConfig);
  MotorDriverCardConfigXML::write(WRITE_FULL_SECOND_HALF_OUTPUT_FILE_NAME,
                                  secondHalfCardConfig);
  MotorDriverCardConfigXML::writeSparse(WRITE_MINIMAL_OUTPUT_FILE_NAME,
                                        defaultCardConfig);
...
void MotorDriverCardConfigXMLTest::testInvalidOutput() {
  BOOST_CHECK_THROW(MotorDriverCardConfigXML::write(
                        "/some/not/existing/file.xml", completeCardConfig),
                    MotorDriverException);
}

void MotorDriverCardConfigXMLTest::testOldInvalidRegister(
  ...
  std::string fileName =
      OLD_INVALID_XML_PREFIX + registerName + OLD_INVALID_XML_SUFFIX;
  BOOST_CHECK_THROW(MotorDriverCardConfigXML::read(fileName),
                    MotorDriverException);
