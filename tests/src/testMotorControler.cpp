...
#define ADD_GET_SET_TEST(NAME)                                                 \
  test_case *set##NAME##TestCase = BOOST_CLASS_TEST_CASE(                      \
      &MotorControlerTest::testSet##NAME, motorControlerTest);                 \
  test_case *get##NAME##TestCase = BOOST_CLASS_TEST_CASE(                      \
      &MotorControlerTest::testGet##NAME, motorControlerTest);                 \
...
#define DEFINE_GET_SET_TEST(NAME, IDX, PATTERN)                                \
  void MotorControlerTest::testGet##NAME() {                                   \
    unsigned int expectedValue =                                               \
        mtca4u::tmc429::testWordFromSpiAddress(_motorControler->getID(), IDX); \
    BOOST_CHECK(_motorControler->get##NAME() == expectedValue);                \
  }                                                                            \
  void MotorControlerTest::testSet##NAME() {                                   \
    _motorControler->set##NAME(PATTERN);                                       \
    BOOST_CHECK(_motorControler->get##NAME() ==                                \
                (PATTERN & mtca4u::tmc429::SPI_DATA_MASK));                    \
  }
...
#define DEFINE_SIGNED_GET_SET_TEST(NAME, IDX, NBITS)                           \
  void MotorControlerTest::testGet##NAME() {                                   \
    ...
    unsigned int expectedValue =                                               \
        mtca4u::tmc429::testWordFromSpiAddress(_motorControler->getID(), IDX); \
    BOOST_CHECK(_motorControler->get##NAME() ==                                \
                converter.customToThirtyTwo(expectedValue));                   \
  }                                                                            \
  void MotorControlerTest::testSet##NAME() {                                   \
    unsigned int positiveMask = (1 << (NBITS - 1)) - 1;                        \
    unsigned int negativeBit = (1 << (NBITS - 1));                             \
    _motorControler->set##NAME(0xAAAAAAAA &                                    \
                               positiveMask); /* something positive */         \
    BOOST_CHECK(_motorControler->get##NAME() ==                                \
                static_cast<int>(0xAAAAAAAA & positiveMask));                  \
    _motorControler->set##NAME((0x55555555 & positiveMask) |                   \
                               negativeBit); /* out of range positive with the \
                                                custom negative bit active*/   \
    BOOST_CHECK(                                                               \
        _motorControler->get##NAME() ==                                        \
        -static_cast<int>((0xAAAAAAAA + 1) &                                   \
                          positiveMask)); /* is interpreted as negative*/      \
    _motorControler->set##NAME(-1);       /* a normal neagative number */      \
    BOOST_CHECK(_motorControler->get##NAME() == -1);                           \
    _motorControler->set##NAME(                                                \
        0xFFFFFFFF & ~negativeBit); /* something negative with the negative    \
                                       bit of the custom variable not set*/    \
    BOOST_CHECK(                                                               \
        _motorControler->get##NAME() ==                                        \
        static_cast<int>(0xFFFFFFFF &                                          \
                         positiveMask)); /* is interpreted as positive */      \
  }
...
#define DEFINE_GET_SET_DRIVER_SPI_DATA(NAME, SPI_ADDRESS, CONFIG_DEFAULT,      \
                                       TEST_PATTERN)                           \
  void MotorControlerTest::testGet##NAME() {                                   \
    testGetDriverSpiData<NAME>(&MotorControlerImpl::get##NAME, SPI_ADDRESS,    \
                               CONFIG_DEFAULT);                                \
  }                                                                            \
  void MotorControlerTest::testSet##NAME() {                                   \
    testSetDriverSpiData<NAME>(&MotorControlerImpl::set##NAME,                 \
                               &MotorControlerImpl::get##NAME, SPI_ADDRESS,    \
                               TEST_PATTERN);                                  \
  }
...
public:
  MotorControlerTest(
      boost::shared_ptr<MotorControler> const &motorControler,
  ...
  // dummy)
  void
  testReadPCIeRegister(unsigned int (MotorControlerImpl::*readFunction)(void),
...
private:
  ...
  template <class T>
  void testGetDriverSpiData(T const &(MotorControlerImpl::*getterFunction)()
                                const,
                            unsigned int driverSpiAddress,
                            unsigned int configDefault);
  template <class T>
  void
  testSetDriverSpiData(void (MotorControlerImpl::*setterFunction)(T const &),
                       T const &(MotorControlerImpl::*getterFunction)() const,
                       unsigned int driverSpiAddress, unsigned int testPattern);

  unsigned int testWordFromPCIeSuffix(std::string const &registerSuffix);
  void listOfMotorControllerPubilcMethods(
      boost::shared_ptr<MotorControler> controller);
  boost::shared_ptr<MotorControler>
  createMotorController(const std::string deviceName);
...
public:
  MotorControlerTestSuite(std::string const &mapFileName, unsigned int motorId)
    ...
    boost::shared_ptr<DFMC_MD22Dummy> dummyDevice =
        boost::dynamic_pointer_cast<DFMC_MD22Dummy>(
    ...
    _motorDriverCard.reset(
        new MotorDriverCardImpl(device, MODULE_NAME_0, motorDriverCardConfig));
    ...
    add(BOOST_TEST_CASE(
        boost::bind(&MotorControlerTest::testGetReferenceSwitchData,
                    motorControlerTest, _motorDriverCard)));
    add(BOOST_CLASS_TEST_CASE(
        &MotorControlerTest::testSetReferenceSwitchEnabled,
        motorControlerTest));
    ...
    add(BOOST_CLASS_TEST_CASE(&MotorControlerTest::testThreadSaftey,
                              motorControlerTest));
    add(BOOST_CLASS_TEST_CASE(&MotorControlerTest::testSetEndSwitchPowerEnabled,
                              motorControlerTest));
...
MotorControlerTest::MotorControlerTest(
    ...
    boost::shared_ptr<DFMC_MD22Dummy> dummyDevice)
    : _motorControler(
          boost::dynamic_pointer_cast<MotorControlerImpl>(motorControler)),
      _registerMapping(registerMapping), _dummyDevice(dummyDevice) {}

DEFINE_SIGNED_GET_SET_TEST(ActualPosition, mtca4u::tmc429::IDX_ACTUAL_POSITION,
                           24)
DEFINE_SIGNED_GET_SET_TEST(ActualVelocity, mtca4u::tmc429::IDX_ACTUAL_VELOCITY,
                           12)
DEFINE_GET_SET_TEST(ActualAcceleration, mtca4u::tmc429::IDX_ACTUAL_ACCELERATION,
                    0xFFFFFF)
DEFINE_GET_SET_TEST(MicroStepCount, mtca4u::tmc429::IDX_MICRO_STEP_COUNT,
                    0xAAAAAA)

void MotorControlerTest::testReadPCIeRegister(
    unsigned int (MotorControlerImpl::*readFunction)(void),
    std::string const &registerSuffix) {
  unsigned int expectedValue = testWordFromPCIeSuffix(registerSuffix);
  std::stringstream message;
  message << "read () " << ((*_motorControler).*readFunction)() << ", expected "
          << expectedValue << std::endl;
  BOOST_CHECK_MESSAGE(((*_motorControler).*readFunction)() == expectedValue,
                      message.str());
}
...
void MotorControlerTest::testThreadSaftey() {
  ...
  auto apiList =
      std::bind(&MotorControlerTest::listOfMotorControllerPubilcMethods, this,
                controller);
...
unsigned int
MotorControlerTest::testWordFromPCIeSuffix(std::string const &registerSuffix) {
  std::string registerName =
      createMotorRegisterName(_motorControler->getID(), registerSuffix);
...
void MotorControlerTest::testReadTypedPCIeRegister(
    T (MotorControlerImpl::*readFunction)(void),
    std::string const &registerSuffix) {
  unsigned int expectedValue = testWordFromPCIeSuffix(registerSuffix);
  std::stringstream message;
  message << "read () " << ((*_motorControler).*readFunction)().getDataWord()
          << ", expected " << expectedValue << std::endl;
  BOOST_CHECK_MESSAGE(((*_motorControler).*readFunction)().getDataWord() ==
                          expectedValue,
                      message.str());
}
...
DEFINE_SIGNED_GET_SET_TEST(TargetPosition, mtca4u::tmc429::IDX_TARGET_POSITION,
                           24)
DEFINE_GET_SET_TEST(MinimumVelocity, mtca4u::tmc429::IDX_MINIMUM_VELOCITY,
                    0xFFFFFF)
DEFINE_GET_SET_TEST(MaximumVelocity, mtca4u::tmc429::IDX_MAXIMUM_VELOCITY,
                    0xAAAAAA)
DEFINE_SIGNED_GET_SET_TEST(TargetVelocity, mtca4u::tmc429::IDX_TARGET_VELOCITY,
                           12)
DEFINE_GET_SET_TEST(MaximumAcceleration,
                    mtca4u::tmc429::IDX_MAXIMUM_ACCELERATION, 0xFFFFFF)
DEFINE_GET_SET_TEST(PositionTolerance,
                    mtca4u::tmc429::IDX_DELTA_X_REFERENCE_TOLERANCE, 0xAAAAAA)
DEFINE_GET_SET_TEST(PositionLatched, mtca4u::tmc429::IDX_POSITION_LATCHED,
                    0x555555)

template <class T>
void MotorControlerTest::testGetTypedData(
    T (MotorControlerImpl::*getterFunction)()) {
  T typedWord;
  unsigned int idx = typedWord.getIDX_JDX();
  // std::cout << idx << std::endl;
  unsigned int expectedContent =
      tmc429::testWordFromSpiAddress(_motorControler->getID(), idx);
  // this assignment checks that the delivered data object actually is the right
  // type of object.
...
template <class T>
void MotorControlerTest::testGetDriverSpiData(
    T const &(MotorControlerImpl::*getterFunction)() const,
    unsigned int driverSpiAddress, unsigned int configDefault) {
  T expectedWord = tmc260::testWordFromSpiAddress(driverSpiAddress,
                                                  _motorControler->getID());

  BOOST_CHECK(_dummyDevice->readDriverSpiRegister(_motorControler->getID(),
                                                  driverSpiAddress) ==
              expectedWord.getPayloadData());
  BOOST_CHECK(((*_motorControler).*getterFunction)() ==
              T(configDefault)); // T is uninitialised at the moment
}

template <class T>
void MotorControlerTest::testSetDriverSpiData(
    void (MotorControlerImpl::*setterFunction)(T const &),
    T const &(MotorControlerImpl::*getterFunction)() const,
    unsigned int driverSpiAddress, unsigned int testPattern) {
  ...
  // the new word must arrive at the spi register in the "hardware" and a copy
  // has to be in the motor controler
  BOOST_CHECK(_dummyDevice->readDriverSpiRegister(_motorControler->getID(),
                                                  driverSpiAddress) ==
...
DEFINE_GET_SET_DRIVER_SPI_DATA(DriverControlData,
                               mtca4u::tmc260::ADDRESS_DRIVER_CONTROL,
                               DRIVER_CONTROL_DEFAULT, 0x2AAAA)
DEFINE_GET_SET_DRIVER_SPI_DATA(ChopperControlData,
                               mtca4u::tmc260::ADDRESS_CHOPPER_CONFIG,
                               CHOPPER_CONTROL_DEFAULT, 0x1AAAA)
DEFINE_GET_SET_DRIVER_SPI_DATA(CoolStepControlData,
                               mtca4u::tmc260::ADDRESS_COOL_STEP_CONFIG,
                               COOL_STEP_CONTROL_DEFAULT, 0x1AAAA)
DEFINE_GET_SET_DRIVER_SPI_DATA(StallGuardControlData,
                               mtca4u::tmc260::ADDRESS_STALL_GUARD_CONFIG,
                               STALL_GUARD_CONTROL_DEFAULT, 0x1AAAA)
DEFINE_GET_SET_DRIVER_SPI_DATA(DriverConfigData,
                               mtca4u::tmc260::ADDRESS_DRIVER_CONFIG,
                               DRIVER_CONFIG_DEFAULT, 0x1AAAA)

void MotorControlerTest::testGetReferenceSwitchData(
    boost::shared_ptr<MotorDriverCardImpl> motorDriverCard) {
  // This approach is intentinally clumsy and manual in order not to use the
  // same algorithm as in the implementation, but still be felxible if the
  // register content changes.
  unsigned int referenceWord =
      0; // setting to 0 to avoid compiler warning in default case.

  switch (_motorControler->getID()) {
  case 0:
    referenceWord = motorDriverCard->getReferenceSwitchData().getDATA() & 0x3;
    break;
  case 1:
    referenceWord =
        ((motorDriverCard->getReferenceSwitchData().getDATA() & 0xC) >> 2);
    break;
  case 2:
    referenceWord =
        ((motorDriverCard->getReferenceSwitchData().getDATA() & 0x30) >> 4);
    break;
  default:
    BOOST_FAIL("Invalid MotorID. Either there is something wrong in the code "
               "or the test has to be adapted.");
  }

  BOOST_CHECK(
      _motorControler->getReferenceSwitchData().getSwitchesActiveWord() ==
      referenceWord);
  BOOST_CHECK(
      _motorControler->getReferenceSwitchData().getPositiveSwitchEnabled() ==
      !_motorControler->getReferenceConfigAndRampModeData()
           .getDISABLE_STOP_R());
  BOOST_CHECK(
      _motorControler->getReferenceSwitchData().getNegativeSwitchEnabled() ==
      !_motorControler->getReferenceConfigAndRampModeData()
           .getDISABLE_STOP_L());
}

void MotorControlerTest::testSetReferenceSwitchEnabled() {
  MotorReferenceSwitchData originalSetting =
      _motorControler->getReferenceSwitchData();

  _motorControler->setPositiveReferenceSwitchEnabled(true);
  _motorControler->setNegativeReferenceSwitchEnabled(true);
  BOOST_CHECK(
      _motorControler->getReferenceSwitchData().getSwitchesEnabledWord() ==
      0x3);
  ...
  _motorControler->setPositiveReferenceSwitchEnabled(
      originalSetting.getPositiveSwitchEnabled());
  _motorControler->setNegativeReferenceSwitchEnabled(
      originalSetting.getNegativeSwitchEnabled());
}

void MotorControlerTest::testTargetPositionReached() {
  ChimeraTK::RegisterInfoMap::RegisterInfo registerInfo;
  _registerMapping->getRegisterInfo(CONTROLER_STATUS_BITS_ADDRESS_STRING,
                                    registerInfo, MODULE_NAME_0);

  TMC429StatusWord expectedControlerStatus(
      testWordFromPCIeAddress(registerInfo.address));

  BOOST_CHECK(expectedControlerStatus.getTargetPositionReached(
                  _motorControler->getID()) ==
...
void MotorControlerTest::testGetReferenceSwitchBit() {
  ...
  BOOST_CHECK(
      expectedControlerStatus.getReferenceSwitchBit(_motorControler->getID()) ==
...
void MotorControlerTest::listOfMotorControllerPubilcMethods(
  ...
  auto controllerImpl =
      boost::dynamic_pointer_cast<MotorControlerImpl>(controller);
  ...
  controllerImpl->setActualAcceleration(
      controllerImpl->getActualAcceleration());
  controllerImpl->setMicroStepCount(controllerImpl->getMicroStepCount());
  controllerImpl->setEnabled();
  controllerImpl->setDecoderReadoutMode(
      controllerImpl->getDecoderReadoutMode());
  ...
  controllerImpl->setMaximumAcceleration(
      controllerImpl->getMaximumAcceleration());
  ...
  controllerImpl->setChopperControlData(
      controllerImpl->getChopperControlData());
  controllerImpl->setCoolStepControlData(
      controllerImpl->getCoolStepControlData());
  controllerImpl->setStallGuardControlData(
      controllerImpl->getStallGuardControlData());
...
MotorControlerTest::createMotorController(const std::string deviceName) {
  ...
  boost::shared_ptr<MotorDriverCardImpl> driver(
      new MotorDriverCardImpl(device, "MD22_0", MotorDriverCardConfig()));
...
test_suite *init_unit_test_suite(int /*argc*/, char * /*argv*/ []) {
  ...
  framework::master_test_suite().add(
      new mtca4u::MotorControlerTestSuite(MAP_FILE_NAME, 0));
  framework::master_test_suite().add(
      new mtca4u::MotorControlerTestSuite(MAP_FILE_NAME, 1));
