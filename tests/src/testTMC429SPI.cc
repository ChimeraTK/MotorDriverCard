...
public:
  TMC429SPITestSuite(std::string const &mapFileName,
    ...
    boost::shared_ptr<TMC429SPITest> tmc429SpiTest(
        new TMC429SPITest(mapFileName, moduleName));

    // in case of dependencies store the test cases before adding them and
    // declare the dependency
    test_case *readTestCase =
        BOOST_CLASS_TEST_CASE(&TMC429SPITest::testRead, tmc429SpiTest);
    test_case *writeTestCase =
        BOOST_CLASS_TEST_CASE(&TMC429SPITest::testWrite, tmc429SpiTest);
...
TMC429SPITest::TMC429SPITest(std::string const &mapFileName,
  ...
  _tmc429Spi.reset(new TMC429SPI(_mappedDevice, moduleName,
...
void TMC429SPITest::testRead() {
  ...
  TMC429OutputWord readResponse =
      _tmc429Spi->read(SMDA_COMMON, JDX_COVER_DATAGRAM);
...
void TMC429SPITest::testWrite() {
  ...
  BOOST_CHECK(_tmc429Spi->read(SMDA_COMMON, JDX_COVER_DATAGRAM).getDATA() ==
              expectedContent);
}
