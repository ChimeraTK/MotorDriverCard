...
#define PRINT_AND_COUNT_ERRORS(TEST_CONDITION, MOTOR_ID)                       \
  if (!(TEST_CONDITION)) {                                                     \
    ...
    std::cout << "\n An error occured evaluating " << #TEST_CONDITION          \
              << " with motorID = " << MOTOR_ID << " in test cycle "           \
              << runCounter << "!" << std::endl;                               \
  }
...
int main(int argc, char *argv[]) {
  if (argc != 4) {
    std::cout
        << argv[0]
        << "This is a long term test which performs SPI communication to the "
           "controler"
        << " and driver chips by writing to the corresponding PCIe regsiters.\n"
        << "It performs evaluation by reading back the written values.\n"
        << "The programme uses the first device listed in the given DMap "
           "file.\n\n"
        << "The test runs forever. Terminate it with <Ctrl><c>\n\n"
        << "Usage: " << argv[0] << " dmapFile moduleName motorConfig.xml"
        << std::endl;
    return -1;
  }

  // get the first device alias from the dmap file and tell the factory to use
  // this dmap file
  ...
  int initialTargetPosition[2] = {motor0->getTargetPosition(),
                                  motor1->getTargetPosition()};
  unsigned int initialMinVelocity[2] = {motor0->getMinimumVelocity(),
                                        motor1->getMinimumVelocity()};
  unsigned int initialMaxVelocity[2] = {motor0->getMaximumVelocity(),
                                        motor1->getMaximumVelocity()};
  unsigned int initialMaxAcceleration[2] = {motor0->getMaximumAcceleration(),
                                            motor1->getMaximumAcceleration()};
  unsigned int initialPositionTolerance[2] = {motor0->getPositionTolerance(),
                                              motor1->getPositionTolerance()};

  while (1) {
    // controler registers (only they can be read back)
    for (unsigned int motorID = 0; motorID <= 1; ++motorID) {
      boost::shared_ptr<mtca4u::MotorControler> motor =
      ...
      motor->setMaximumAcceleration(
          MASK24(initialMaxAcceleration[motorID] + 3));
      motor->setPositionTolerance(
          MASK24(initialPositionTolerance[motorID] + 4));

      PRINT_AND_COUNT_ERRORS(motor->getTargetPosition() ==
                                 MASK24(initialTargetPosition[motorID] + 1000),
                             motorID);
      PRINT_AND_COUNT_ERRORS(motor->getMinimumVelocity() ==
                                 MASK24(initialMinVelocity[motorID] + 1),
                             motorID);
      PRINT_AND_COUNT_ERRORS(motor->getMaximumVelocity() ==
                                 MASK24(initialMaxVelocity[motorID] + 2),
                             motorID);
      PRINT_AND_COUNT_ERRORS(motor->getMaximumAcceleration() ==
                                 MASK24(initialMaxAcceleration[motorID] + 3),
                             motorID);
      PRINT_AND_COUNT_ERRORS(motor->getPositionTolerance() ==
                                 MASK24(initialPositionTolerance[motorID] + 4),
                             motorID);

      // set back to the initial values (another test plus needed for the next
      // test round because we have to alternate the register content. This time
      // we write in reverse order, just for the fun of it
      ...
      PRINT_AND_COUNT_ERRORS(motor->getTargetPosition() ==
                                 initialTargetPosition[motorID],
                             motorID);
      PRINT_AND_COUNT_ERRORS(
          motor->getMinimumVelocity() == initialMinVelocity[motorID], motorID);
      PRINT_AND_COUNT_ERRORS(
          motor->getMaximumVelocity() == initialMaxVelocity[motorID], motorID);
      PRINT_AND_COUNT_ERRORS(motor->getMaximumAcceleration() ==
                                 initialMaxAcceleration[motorID],
                             motorID);
      PRINT_AND_COUNT_ERRORS(motor->getPositionTolerance() ==
                                 initialPositionTolerance[motorID],
                             motorID);
    }

    // patterns for both motors, including writing to controler and driver
    // registers, where only controler registers can be checked. Nevertheless
    ...
    // This is because the value is interpreted as signed 24 bit, and the return
    // value is the corresponding signed 32 bit integer.
    PRINT_AND_COUNT_ERRORS(
        motor0->getTargetPosition() == static_cast<int32_t>(0xFFAAAAAA), 0);
    PRINT_AND_COUNT_ERRORS(
        motor1->getTargetPosition() == static_cast<int32_t>(0x555555), 1);
    ...
    PRINT_AND_COUNT_ERRORS(
        motor0->getTargetPosition() == initialTargetPosition[0], 0);
    PRINT_AND_COUNT_ERRORS(
        motor1->getTargetPosition() == initialTargetPosition[1], 1);
    PRINT_AND_COUNT_ERRORS(
        motor0->getPositionTolerance() == initialPositionTolerance[1], 1);
    PRINT_AND_COUNT_ERRORS(
        motor1->getPositionTolerance() == initialPositionTolerance[0], 0);
    ...
    if (currentTime > lastPrintTime) {
      std::cout << "\r" << currentTime - startingTime << " second"
                << (currentTime - startingTime != 1 ? "s" : "") << " elapsed, "
                << runCounter << " test cycles completed, " << errorCounter
                << " errors detected so far." << std::flush;
