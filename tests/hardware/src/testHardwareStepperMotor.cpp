...
void* statusReaderThreadBody(void* ptr) {
  ...
  while(1) {
    std::cout << "Thread:: " << (unsigned int)(pthread_self())
              << " motor status is: " << motorPointer->getStatusAndError().status.asString() << std::endl;
    usleep(10000);
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Try to open MOTOR CARD with device name: " << deviceName << " with config file: " << configFileNameName
  ...
  if(motor.getCalibrationStatus() == mtca4u::StepperMotorCalibrationStatusType::M_CALIBRATED) {
    ...
    std::cout << "Calibration finished with status: " << motor.getCalibrationStatus() << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
              << std::endl;
